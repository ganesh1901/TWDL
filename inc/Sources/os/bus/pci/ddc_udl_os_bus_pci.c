...
#define DYNAMICLLY_ALLOC_MAJOR_NUMBER       0

#define MODULE_NAME                         "acexpci"

static int nMajorNumber = 0;
...
)
{
    ...
    if (0 > (result = register_chrdev(DYNAMICLLY_ALLOC_MAJOR_NUMBER, MODULE_NAME, &ddcUdlHookFileOperations)))
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_BUS, DDC_DBG_BUS_INIT, "register_chrdev() returned %d\n", result);
        return (S16BIT) result;
    }

    /* registration succeeded, use the major number it returned */
    if (nMajorNumber == 0)
    {
        nMajorNumber = result;
    }
    ...
    {
        /* ERROR: release the major number */
        unregister_chrdev(nMajorNumber, MODULE_NAME);
...
)
{
    ...
    for (u8CardNumber = 0; u8CardNumber < u8CardCount; u8CardNumber++)
    {
...
)
{
    ...
    {
        *pu32RdData = WORD_ORDER_L(_ddcUdlOsBusReadU32bit(pDeviceContext,
...
)
{
    ...
    if (u32Mode == ACEX_32_BIT_ACCESS_16_BIT_HW_MEM)
    {
        _ddcUdlOsBusWriteU32bit(pDeviceContext, WORD_ORDER_L(*pu32WrData),
            (U8BIT *)(pDeviceContext->ddcOsDevInfo.sBusInfo.pMemAddr + u32Addr));
    }
    else
    {
...
)
{
    ...
    for (i = 0; i < u32Count; i++, pu32WrData++)
    {
        ...
        {
            _ddcUdlOsBusWriteU32bit(pDeviceContext,
                WORD_ORDER_L(*pu32WrData), (U8BIT *)(pDeviceContext->ddcOsDevInfo.sBusInfo.pMemAddr + u32Addr + (i * 4)));
        }
        else
        {
            _ddcUdlOsBusWriteU32bit(pDeviceContext, *pu32WrData,
...
)
{
    ...
    if (u32Start16Addr)
    {
        ...
        u32TempData = WORD_ORDER_L(_ddcUdlOsBusReadU32bit(pDeviceContext,
        ...
        /* write the data back */
        _ddcUdlOsBusWriteU32bit(pDeviceContext,
            WORD_ORDER_L(u32TempData), (U8BIT *)pu32BlkMemAddr);
    ...
    if (u32End16Addr)
    {
        ...
        /* write the data back */
        _ddcUdlOsBusWriteU32bit(pDeviceContext, WORD_ORDER_L(u32TempData),
            (U8BIT *)(pu32BlkMemAddr));
    }
