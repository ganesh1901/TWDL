...
)
{
    ...
    U32BIT u32IoctlParamSize;

    struct _DDC_IOCTL_PARAMS sIoctlParams;
    struct _DDC_COMPAT_IOCTL_PARAMS sCompatIoctlParams;
    
    struct _DDC_IOCTL_PARAMS        *pIoctlParams       = &sIoctlParams;
    struct _DDC_COMPAT_IOCTL_PARAMS *pCompatIoctlParams = &sCompatIoctlParams;
    struct _DDC_UDL_DEVICE_CONTEXT  *pDeviceContext     = NULL;
    S16BIT s16Result    = 0;
    int    linux_result = LINUX_SUCCESS;

    u8DeviceIndex   = MINOR(inode->i_rdev);
    u8CardNumber    = (U8BIT)(u8DeviceIndex / MAX_NUM_1553_CHANNELS);
    u8ChannelNumber = u8DeviceIndex % MAX_NUM_1553_CHANNELS;

    /* check if the ARINC library is accessing the driver */
    if (u8DeviceIndex >= ARINC_MINOR && u8DeviceIndex < DISCRETE_MINOR)
    {
        u8CardNumber = u8DeviceIndex - ARINC_MINOR;
    }

    /* check if the I/0 library is accessing the driver */
    if (u8DeviceIndex >= DISCRETE_MINOR)
    {
        u8CardNumber = u8DeviceIndex - DISCRETE_MINOR;
    }
    ...
    if (ioctl_param)
    {
        if (u8IsCompatCall)
        {
            s16Result = (S16BIT)DDC_COPY_FROM_USER((char *)&sCompatIoctlParams, (char *)ioctl_param, sizeof(struct _DDC_COMPAT_IOCTL_PARAMS));

            u32InputBufferLength = (U32BIT)sCompatIoctlParams.sIoctlInfo.u32InputBufferLength;
            u32IoctlParamSize    = (U32BIT)sizeof(struct _DDC_COMPAT_IOCTL_PARAMS);
        }
        else
        {
            s16Result = (S16BIT)DDC_COPY_FROM_USER((char *)&sIoctlParams, (char *)ioctl_param, sizeof(struct _DDC_IOCTL_PARAMS));

            u32InputBufferLength = (U32BIT)sIoctlParams.sIoctlInfo.u32InputBufferLength;
            u32IoctlParamSize    = (U32BIT)sizeof(struct _DDC_IOCTL_PARAMS);
        }
        ...
        if (u8IsCompatCall)
        {
            pIoctlParams->Channel = (unsigned long )pCompatIoctlParams->Channel;

            pIoctlParams->Param1  = (unsigned long )pCompatIoctlParams->Param1;
            pIoctlParams->Param2  = (unsigned long )pCompatIoctlParams->Param2;
            pIoctlParams->Param3  = (unsigned long )pCompatIoctlParams->Param3;
            pIoctlParams->Param4  = (unsigned long )pCompatIoctlParams->Param4;
            pIoctlParams->Param5  = (unsigned long )pCompatIoctlParams->Param5;
            pIoctlParams->Param6  = (unsigned long )pCompatIoctlParams->Param6;

            pIoctlParams->pInBuffer = (unsigned long )pCompatIoctlParams->pInBuffer;
    
            pIoctlParams->sIoctlInfo.u32InputBufferLength   = (unsigned long )pCompatIoctlParams->sIoctlInfo.u32InputBufferLength;
            pIoctlParams->sIoctlInfo.pOutBuffer             = (unsigned long )pCompatIoctlParams->sIoctlInfo.pOutBuffer;
            pIoctlParams->sIoctlInfo.u32OutBufferSize       = (unsigned long )pCompatIoctlParams->sIoctlInfo.u32OutBufferSize;
            pIoctlParams->sIoctlInfo.pu32BytesReturned      = (unsigned long )pCompatIoctlParams->sIoctlInfo.pu32BytesReturned;
            pIoctlParams->sIoctlInfo.status                 = (long )pCompatIoctlParams->sIoctlInfo.status;
        }

        /* check if the ioctl cmd is valid */
        if (u32InputBufferLength > u32IoctlParamSize)
        {
             DDC_DBG_PRINT(DDC_DBG_MODULE_DRIVER, DDC_DBG_DEVICE_NOTIFY, 
                    "IOCTL CMD error: cmd %d  subcmd %ld  u8IsCompatCall %d >>>> u32InputBufferLength %d > u32IoctlParamSize %d\n", 
                    ioctl_num, pIoctlParams->Param1, u8IsCompatCall, u32InputBufferLength, u32IoctlParamSize); 
            return -EFAULT;
        }
        ...
        /* execute commands */
        s16Result = ddcUdlIoctl(pDeviceContext, pIoctlParams, ioctl_num, u8CardNumber, u8ChannelNumber, u8IsCompatCall);
    
        /* free larger IOCTL command buffer */
        if (pIoctlParams != &sIoctlParams)
        {
            DDC_KERNEL_FREE(pIoctlParams);
        }

        if (u8IsCompatCall)
        {
            if (pCompatIoctlParams != &sCompatIoctlParams)
            {
                DDC_KERNEL_FREE(pCompatIoctlParams);
            }
        }
...
)
{
    ...
    U8BIT u8DevIndx = MINOR(inode->i_rdev);
    U8BIT u8CardNumber = (U8BIT)(u8DevIndx / MAX_NUM_1553_CHANNELS);
    U8BIT u8ChannelNumber = u8DevIndx % MAX_NUM_1553_CHANNELS;
    U8BIT u8CardCount = 0;
    U8BIT u8ChannelType;

    DDC_DBG_PRINT(DDC_DBG_MODULE_DRIVER, DDC_DBG_DEVICE_OPEN,
        "ENTER: minor %d  u8CardNumber %d  u8ChannelNumber %d\n",
        u8DevIndx, u8CardNumber, u8ChannelNumber);

    /* check if the ARINC library is making the access */
    if (u8DevIndx >= ARINC_MINOR && u8DevIndx < DISCRETE_MINOR)
    {
        /* adjust the card number accordingly */
        u8CardNumber = u8DevIndx - ARINC_MINOR;
    ...
    else if (u8DevIndx >= DISCRETE_MINOR) /* check if the discrete library is making the access */
    {
        /* adjust the card number accordingly */
        u8CardNumber = u8DevIndx - DISCRETE_MINOR;
    ...
    if ( ( (u8DevIndx >= ARINC_MINOR) && (u8DevIndx < DISCRETE_MINOR) && pDeviceContext->u8ArincSerialOpen)   ||
         ( (u8DevIndx >= DISCRETE_MINOR) && pDeviceContext->u8DiscreteOpen)   ||
         ( (u8DevIndx < ARINC_MINOR) && pDeviceContext->u8Channel1553Open[u8ChannelNumber]) )
    {
        /* Release Mutex */
    ...
    else if (u8DevIndx >= DISCRETE_MINOR)
    {
