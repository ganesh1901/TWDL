...
)
{
    ...
    pDeviceContext->sDioTt.pu8DmaTarget = DDC_DMA_ALLOC(
        pDeviceContext,
...
)
{
    ...
    if (pDeviceContext->sDioTt.pu8DmaTarget != NULL)
    {
        DDC_DMA_FREE(
            pDeviceContext,
            *pDeviceContext->sDioTt.pu32MemSize, /* //todo - check size from above - <<2 needed? */
