...
)
{
    ...
    if (sCapabilityInfo.channelCountCanBus > 0)
    {
        ...
        {
            DDC_DBG_PRINT(DDC_DBG_MODULE_CAN, DDC_DBG_CANBUS_INITIALIZE, "canBusInitialize ERROR: status:%d\n", status);
            return (S16BIT)status;
        }
#endif
    }
...
)
{
    ...
    U32BIT u32State = (U32BIT)pConfig->eState;
    ...
    switch (u32State)
    {
        ...
        {
            /* Save State */
            pDeviceContext->eCanBusState[u8Channel] = u32State;
            break;
        }
...
)
{
    ...
    U32BIT i = 0;
    U32BIT u8MsgCount = pIoctlParams->Param1;
    ...
    for (i = 0; i < u8MsgCount; i++)
    {
        ...
        if ((pDeviceContext->sCanBus.u32MemTxIndex[u8Channel] + CAN_BUS_DWORDS_PER_MSG) > CAN_BUS_MEM_TX_SIZE)
        {
            ...
            u8DWordsTx = (CAN_BUS_MEM_TX_SIZE - u8DWordsTx);
...
)
{
    ...
    if (u8Channel < pDeviceContext->u8NumCanBus)
    {
        ...
        if (sHbuf->hMemory == NULL)
        {
            ...
            /* Create host buffer */
            sHbuf->hMemory = DDC_KERNEL_VIRTUAL_MALLOC(u32BufferByteSize);
            if (!sHbuf->hMemory)
            {
            ...
            else
            {
                ...
                sHbuf->hUsbReadBuffer = DDC_KERNEL_MALLOC(u32BufferByteSize);
...
)
{
    ...
    CAN_HBUF_TYPE *sHbuf;
    U8BIT u8Channel = (U8BIT)pIoctlParams->Channel - 1;
    U32BIT u32NumMsgsToRead = pIoctlParams->Param1;
    ...
    *pOutputBufferLength = numBytesToCopy;
...
)
{
    ...
    if (u8Channel < pDeviceContext->u8NumCanBus)
    {
        ...
        if (sHbuf->hMemory != NULL)
        {
            ...
            DDC_KERNEL_VIRTUAL_FREE(sHbuf->hMemory);
            ...
            {
                DDC_KERNEL_FREE(sHbuf->hUsbReadBuffer);
