...
{ \
    ...
    if (pDeviceContext->b429MemSwByteSwap) \
    { \
        u32WriteData = BYTE_ORDER_L(u32WriteData); \
    } \
    \
...
{ \
    ...
    if (pDeviceContext->b429MemSwByteSwap) \
    { \
        u32Data = BYTE_ORDER_L(u32Data); \
    } \
    DDC_MEM_WRITE(pDeviceContext, *(pDeviceContext->sArinc429TxGlobal.pu32MemBA) + offset, &u32Data, ACEX_32_BIT_ACCESS); \
}
...
)
{
    ...
    {
        pDeviceContext->pRxChnl429[i] = DDC_KERNEL_MALLOC(sizeof(ACEX_429_RX_TYPE));
        pDeviceContext->pTxChnl429[i] = DDC_KERNEL_MALLOC(sizeof(ACEX_429_TX_TYPE));
    ...
    pDeviceContext->p429TxMemory = DDC_KERNEL_MALLOC(1024 * sizeof(U32BIT));
    ...
    if ((pDeviceContext->u16DeviceID == DDC_DEV_ID_BU67118FMX) ||
...
)
{
    ...
    {
        DDC_KERNEL_FREE(pDeviceContext->p429TxMemory);
...
#endif /* DDC_TRANSFER_VIA_DMA_429 */
    ...
    for (i = 0; i < MAX_NUM_429_CHANNELS; i++)
    {
        ...
        {
            DDC_KERNEL_FREE(pDeviceContext->pRxChnl429[i]);
            pDeviceContext->pRxChnl429[i] = NULL;
        }

        if (pDeviceContext->pTxChnl429[i])
        {
            DDC_KERNEL_FREE(pDeviceContext->pTxChnl429[i]);
            pDeviceContext->pTxChnl429[i] = NULL;
        }
    }
}
...
)
{
    ...
    u32Format = pIoctlParams->Param1;
    u32Rollover = pIoctlParams->Param2;
    u32Resolution = pIoctlParams->Param3;
...
)
{
    ...
    if (pPciIo->Param1 < START_SCH_MEMORY)
    {
        ...
        u32MemoryOffset = (pPciIo->Param1 - MIO_RX_FIFOMB_0) % MIO_RX_FIFOMB_LEN;
    ...
    else if (pPciIo->Param1 < START_DMT_MEMORY)
    {
        ...
        u32MemoryOffset = pPciIo->Param1 - START_SCH_MEMORY;
        u32MemoryAddress = *(pDeviceContext->sArinc429TxGlobal.pu32MemBA) + u32MemoryOffset;
    }
    else
    {
        /* DMT address */
        u32MemoryOffset = pPciIo->Param1 - START_DMT_MEMORY;
        u32MemoryAddress = *(pDeviceContext->sArinc429RxGlobal.pu32MemBA) + u32MemoryOffset + 0x00002000;
    }
    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_TRANSLATE_MEMORY_ADDR, "429 - ARINC429GetMemoryAddress:In 0x%08lX, Out 0x%08X\n", pPciIo->Param1, u32MemoryAddress);
...
)
{
    ...
    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_TRANSLATE_REG_ADDR, "429 - ARINC429GetRegisterAddress:In 0x%08lX, Out 0x%08X\n", pPciIo->Param1, u32RegisterAddress);
...
)
{
    ...
    if (UART_PROTOCOL == pIoctlParams->Param2)
    {
        ...
        u32TempReg = ((0x0000000f & pIoctlParams->Param3) << (bitPostiion * 4));
    ...
    else if (UART_INTERRUPT_ENABLE == pIoctlParams->Param2)
    {
        ...
        */
        ...
        u32TempRegValue = pIoctlParams->Param3;
        DDC_REG_WRITE(pDeviceContext,  *(pDeviceContext->sMioCastUartGlobal429.pu32RegBA) + u32TempOFFSET, &u32TempRegValue);
    }
    else if (UART_PHYSICAL == pIoctlParams->Param2)
    {
        ...
        */
        if (u32PairedTranceivers) /* NEED TO UPDATE THIS - bu67118 transceiver pairing */
        {
            ...
            bitPostiion = (u32tempChan * 4);
            u32TempReg = ((0x0000000f & pIoctlParams->Param3) << bitPostiion);
        ...
        else
        {
            ...
            switch(pDeviceContext->castSerialIOChanMatrix[u16Channel].castSerialMode)
            {
                case CAST_PROTOCOL_SDLC:
                {
                    ...
                    {
                        DDC_REG_READ(pDeviceContext, *(pDeviceContext->castSerialIOChanMatrix[u16Channel].serialSDLC.pu32RegBA) + (pPciIoRead->Param2), &u32RegisterValue);
                        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_SERIAL_IO, "429 - ARINC429CastSerialIORegRead - UART: Address: 0x%08lX, Value: 0x%08X\n", *(pDeviceContext->castSerialIOChanMatrix[u16Channel].serialSDLC.pu32RegBA) + (pPciIoRead->Param2), u32RegisterValue);
                    }
                ...
                case CAST_PROTOCOL_HDLC:
                {
                    ...
                    {
                        DDC_REG_READ(pDeviceContext, *(pDeviceContext->castSerialIOChanMatrix[u16Channel].serialHDLC.pu32RegBA) + (pPciIoRead->Param2), &u32RegisterValue);
                        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_SERIAL_IO, "429 - ARINC429RegRead - UART: Address: 0x%08lX, Value: 0x%08X\n", *(pDeviceContext->castSerialIOChanMatrix[u16Channel].serialHDLC.pu32RegBA) + (pPciIoRead->Param2), u32RegisterValue);
                    }
                ...
                default:
                {
                    ...
                    {
                        DDC_REG_READ(pDeviceContext, *(pDeviceContext->castSerialIOChanMatrix[u16Channel].serialAsyncUart.pu32RegBA) + (pPciIoRead->Param2 & 0x00FF), &u32RegisterValue);
...
)
{
        ...
        (pDeviceContext->u8NumUart > 0))
    {
        ...
        {
            DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_SERIAL_IO, "429 - CAST_PROTOCOL_UART - castSerialMode: %x u16Channel %d  p2 %lx p3 %lx\n", pDeviceContext->castSerialIOChanMatrix[u16Channel].castSerialMode, u16Channel, pIoctlParams->Param2, pIoctlParams->Param3);
        ...
        /* first check to see how this channel is configured */
            switch(pDeviceContext->castSerialIOChanMatrix[u16Channel].castSerialMode)
            {
                case CAST_PROTOCOL_SDLC:
                {
                    ...
                    {
                        u32TempRegValue = pIoctlParams->Param3;
                        DDC_REG_WRITE(pDeviceContext, *(pDeviceContext->castSerialIOChanMatrix[u16Channel].serialSDLC.pu32RegBA) + (pIoctlParams->Param2 & 0x00FF), &u32TempRegValue);
                        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_SERIAL_IO, "429 - ARINC429RegWrite - SDLC: Address: 0x%08lX, Value: 0x%08lX\n", *(pDeviceContext->castSerialIOChanMatrix[u16Channel].serialSDLC.pu32RegBA) + (pIoctlParams->Param2 & 0x00FF), pIoctlParams->Param3);
                    }
                ...
                case CAST_PROTOCOL_HDLC:
                {
                    ...
                    {
                        u32TempRegValue = pIoctlParams->Param3;
                        DDC_REG_WRITE(pDeviceContext, *(pDeviceContext->castSerialIOChanMatrix[u16Channel].serialHDLC.pu32RegBA) + (pIoctlParams->Param2 & 0x00FF), &u32TempRegValue);
                        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_SERIAL_IO, "429 - ARINC429RegWrite - HDLC: Address: 0x%08lX, Value: 0x%08lX\n", *(pDeviceContext->castSerialIOChanMatrix[u16Channel].serialHDLC.pu32RegBA) + (pIoctlParams->Param2 & 0x00FF), pIoctlParams->Param3);
                    }
                ...
                default:
                {
                    ...
                    {
                        u32TempRegValue = pIoctlParams->Param3;
                        DDC_REG_WRITE(pDeviceContext, *(pDeviceContext->castSerialIOChanMatrix[u16Channel].serialAsyncUart.pu32RegBA) + (pIoctlParams->Param2 & 0x00FF), &u32TempRegValue);
                        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_SERIAL_IO, "429 - CAST_PROTOCOL_UART - UART: Address: 0x%08lX, Value: 0x%08lX\n", *(pDeviceContext->castSerialIOChanMatrix[u16Channel].serialAsyncUart.pu32RegBA) + (pIoctlParams->Param2 & 0x00FF), pIoctlParams->Param3);
                    }
...
)
{
    ...
    if ((pPciIoRead->Param1 >= MIO_UART_1) && (pPciIoRead->Param1 <= MIO_UART_IO_CTRL))
    {
            ...
            (pDeviceContext->u8NumUart > 0))
        {
            switch (pPciIoRead->Param1)
            {
                ...
                default:
                {
                    DDC_REG_READ(pDeviceContext, *(pDeviceContext->sMioUart429.pu32RegBA) + (pPciIoRead->Param1 & 0x00FF), &u32RegisterValue);
                    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_READ_REGISTER,  "429 - ARINC429RegRead - UART: Address: 0x%08lX, Value: 0x%08X\n", *(pDeviceContext->sMioUart429.pu32RegBA) + (pPciIoRead->Param1 & 0x00FF), u32RegisterValue);
                    break;
                }
            }
        }
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_READ_REGISTER,  "429 - ARINC429RegRead Board Address: 0x%08lX\n", pPciIoRead->Param1);
...
)
{
            ...
            (pDeviceContext->u8NumUart > 0))
        {
            switch (pIoctlParams->Param1)
            {
                case MIO_UART_1:
                {
                    ...
                    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_WRITE_REGISTER, "429 - ARINC429RegWrite - UART: Address: 0x%08X, Value: 0x%08lX\n", *(pDeviceContext->sMioUart429.pu32RegBA) + (MIO_UART_1 & 0x00FF), pIoctlParams->Param2);
                    break;
                }
                ...
                default:
                {
                    DDC_REG_WRITE(pDeviceContext, *(pDeviceContext->sMioUart429.pu32RegBA) + (pIoctlParams->Param1 & 0x00FF), (U32BIT *)&u32RegisterValue);
                    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_WRITE_REGISTER, "429 - ARINC429RegWrite - UART: Address: 0x%08lX, Value: 0x%08lX\n", *(pDeviceContext->sMioUart429.pu32RegBA) + (pIoctlParams->Param1 & 0x00FF), pIoctlParams->Param2);
                    break;
                }
            }
        }
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_WRITE_REGISTER, "429 - ARINC429RegWrite Board Address: 0x%08lX\n", pIoctlParams->Param1);
        ...
        switch (pIoctlParams->Param1)
        {
            ...
            case MIO_IRIG_CFG_A:
            {
                DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_WRITE_REGISTER, "429 - ARINC429RegWrite Address: 0x%08X, Value: 0x%08lX\n", *(pDeviceContext->sIrigB_RX->pu32RegBA) + REG_IRIGB_CONTROL_RW, pIoctlParams->Param2);
                ARINC429WriteIRIGConfigRegister(pDeviceContext, pIoctlParams->Param2);
                break;
            }
            ...
            case MIO_429_RESET:
            {
                ...
                u32RegisterValue = pIoctlParams->Param2;
                u32RegisterValue <<= 14;
    ...
    else
    {
        ...
        if ((pIoctlParams->Param1 >= MIO_429_RX_CTRL_0) && (pIoctlParams->Param1 <= MIO_429_RX_CTRL_15))
        {
            ...
            if ((((pIoctlParams->Param1 - START_RX_REG_ADDRESS) % 3) == 2) && (((~pIoctlParams->Param2) & mask) == mask))
            {
                ...
                ARINC429ConfigInternalRxInterrupt(pDeviceContext, (pIoctlParams->Param1 - START_RX_REG_ADDRESS) / 3);
            }
        }
    }
}
...
)
{
    ...
    u32RegisterValue = (U32BIT)pIoctlParams->Param2;
    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_WRITE_MEMORY, "429 - ARINC429MemWrite Address: 0x%08X, Value: 0x%08lX\n", u32MemoryAddress, pIoctlParams->Param2);
    /* NOTE: PPC byte swap is done in IOCTL */
 ...
 *
 * In:  pDeviceContext      device-specific structure
 * In:  pIoctlParams           ->Channel   Chanel to operate on (N/A for board commands)
 *                          ->Param1    Command
 *                          ->Param2    Command specific value
 *                          ->Param3    Command specific value
 *                          ->Param4    Command specific value
 * In:  pRdData             pointer to output data
...
)
{
    ...
    U32BIT u32Command = pIoctlParams->Param1;

    /* default the response to success */
    *pRdData = (U32BIT) DDC_UDL_ERROR__SUCCESS;

    /* default to only having the response being returned */
    *pBytesReturned = sizeof(U32BIT);

    if ((u32Command >= DD429_GENERAL_COMMAND__INDEX_START) &&
        (u32Command <= DD429_GENERAL_COMMAND__INDEX_END))
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_PROCESS, "429 - ARINC429ProcessCommand(CH:%ld) - General Command %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
        status = _ARINC429GeneralCommand(pDeviceContext, pIoctlParams, pRdData, OutputBufferLength, pBytesReturned);
    }
    else if ((u32Command >= DD429_CONTROL_COMMAND__INDEX_START) &&
        (u32Command <= DD429_CONTROL_COMMAND__INDEX_END))
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_PROCESS, "429 - ARINC429ProcessCommand(CH:%ld) - Control Command %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
        status = _ARINC429ControlCommand(pDeviceContext, pIoctlParams, pRdData, OutputBufferLength, pBytesReturned);
    }
    else if ((u32Command >= DD429_FIFO_COMMAND__INDEX_START) &&
        (u32Command <= DD429_FIFO_COMMAND__INDEX_END))
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_PROCESS, "429 - ARINC429ProcessCommand(CH:%ld) - FIFO Command %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
        status = _ARINC429FifoCommand(pDeviceContext, pIoctlParams, pRdData, OutputBufferLength, pBytesReturned);
    }
    else if ((u32Command >= DD429_RX_COMMAND__INDEX_START) &&
        (u32Command <= DD429_RX_COMMAND__INDEX_END))
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_PROCESS, "429 - ARINC429ProcessCommand(CH:%ld) - RX Command %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
        status = _ARINC429RxCommand(pDeviceContext, pIoctlParams, pRdData, OutputBufferLength, pBytesReturned);
    }
    else if ((u32Command >= DD429_TX_COMMAND__INDEX_START) &&
        (u32Command <= DD429_TX_COMMAND__INDEX_END))
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_PROCESS, "429 - ARINC429ProcessCommand(CH:%ld) - Tx Command %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
        status = _ARINC429TxCommand(pDeviceContext, pIoctlParams, pRdData, OutputBufferLength, pBytesReturned);
    }
    else if ((u32Command >= DD429_RX_CONTROL_COMMAND_SET__INDEX_START) &&
        (u32Command <= DD429_RX_CONTROL_COMMAND_SET__INDEX_END))
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_PROCESS, "429 - ARINC429ProcessCommand(CH:%ld) - RX Control Command Set %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_PROCESS, "429 - ARINC429ProcessCommand(CH:%ld) - RX Control Command Get %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
        status = _ARINC429RxControlCommandGet(pDeviceContext, pIoctlParams, pRdData, OutputBufferLength, pBytesReturned);
    }
    else if ((u32Command >= DD429_FILTER_COMMAND__INDEX_START) &&
        (u32Command <= DD429_FILTER_COMMAND__INDEX_END))
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_PROCESS, "429 - ARINC429ProcessCommand(CH:%ld) - Filter Command %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
        status = _ARINC429FilterCommand(pDeviceContext, pIoctlParams, pRdData, OutputBufferLength, pBytesReturned);
    }
    else if ((u32Command >= DD429_MAILBOX_COMMAND__INDEX_START) &&
        (u32Command <= DD429_MAILBOX_COMMAND__INDEX_END))
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_PROCESS, "429 - ARINC429ProcessCommand(CH:%ld) - mailbox Command %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
        status = _ARINC429MailboxCommand(pDeviceContext, pIoctlParams, pRdData, OutputBufferLength, pBytesReturned);
    }
    else if ((u32Command >= DD429_TESTER_COMMAND__INDEX_START) &&
        (u32Command <= DD429_TESTER_COMMAND__INDEX_END))
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_PROCESS, "429 - ARINC429ProcessCommand(CH:%ld) - tester Command %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
        status = _ARINC429TesterCommand(pDeviceContext, pIoctlParams, pRdData, OutputBufferLength, pBytesReturned);
    }
    else
    {
        /* ERROR: unknown command */
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_PROCESS, "429 - ARINC429ProcessCommand(CH:%ld) - Unknown Command %ld !!!\n", pIoctlParams->Channel, pIoctlParams->Param1);

        *pRdData = (U32BIT) ERR_FEATURE_NOT_SUPPORTED;
    }
 ...
 * Description:
 *
 *      This function converts a tester options structue into a 32-bit register
 *      value.
 *
...
)
{
    ...
    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_RX, "429 - _ARINC429RxCommand(CH:%ld) - Cmd %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
    ...
    switch (pIoctlParams->Param1)
    {
        ...
        /* ---------------------------------------------------- */
        {
            ...
            U32BIT u32EnableValue = pIoctlParams->Param2;
            U32BIT u32PassiveValue = pIoctlParams->Param3;

            sConfig.u32ConfigOption = ARINC_429_PROGRMMABLE_TYPE_OPT;
            sConfig.bEnable = (U8BIT)u32EnableValue;
            sConfig.sConfigID.u16Channel = u16Channel;
        ...
        /* ---------------------------------------------------- */
        {
            DD429_GET_TIMESTAMP__OUTPUT_TYPE *psTimeStampValue = (DD429_GET_TIMESTAMP__OUTPUT_TYPE *) pRdData;
        ...
        /* ---------------------------------------------------- */
        {
            DD429_GET_TIME_STAMP_CONFIG__OUTPUT_TYPE *psGetTimeStampConfigOutput = (DD429_GET_TIME_STAMP_CONFIG__OUTPUT_TYPE *) pRdData;
        ...
        /* ---------------------------------------------------- */
        {
            *pRdData = _ARINC429GetLoopbackMapping(pDeviceContext, pIoctlParams->Channel);
            break;
        }
        ...
        /* ---------------------------------------------------- */
        {
            _ARINC429SetLoopbackMapping(pDeviceContext, pIoctlParams->Param2, pIoctlParams->Param3, FALSE);
            break;
        }
        ...
        /* ---------------------------------------------------- */
        {
            U32BIT u32Type = pIoctlParams->Param2;
            U32BIT u32Condition = pIoctlParams->Param3;
            U32BIT u32RegisterAddress;
            ...
            switch (u32Type)
            {
                ...
                /* --------------------- */
                {
                    ...
                    if (sIoctlParams.Param3)
                    {
                        ...
                        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_RX, "429 - _ARINC429RxCommand bdInterruptSet sIoctlParams.Param3 %lx  channel %x\n", sIoctlParams.Param3, u16Channel);
                    }
                    else
                    {
                        if (u16Channel < 32)
                        ...
                        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_RX, "429 - _ARINC429RxCommand bdInterruptClear sIoctlParams.Param3 %lx  channel %x\n", sIoctlParams.Param3, u16Channel);
                    }
        ...
        {
            /* unknown type */
            DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_RX, "429 - _ARINC429RxCommand(CH:%ld) - Unknown Command %ld !!!\n", pIoctlParams->Channel, pIoctlParams->Param1);
            *pRdData = (U32BIT) ERR_UNKNOWN;
            break;
        }
    }
...
)
{
    ...
    u32SettingType = pIoctlParams->Param1;
    u32SettingValue = pIoctlParams->Param2;
 ...
 * In:  pIoctlParams               pointer to IOCTL command structure
 * In:  OutputBufferLength      length of output buffer in bytes
 * Out: pRdData                 value of command item
 * Out: pBytesReturned          pointer to number of bytes retuned in the output buffer
 * Out: return                  error condition
 ******************************************************************************/
static S16BIT _ARINC429RxControlCommandGet
...
{
    DD429_RX_CONTROL_COMMAND_GET__OUTPUT_TYPE *psRxControlCommandGetOutput = (DD429_RX_CONTROL_COMMAND_GET__OUTPUT_TYPE *) pRdData;
    U32BIT u32RegisterAddress = 0;
    U32BIT u32BitOffset;
    U32BIT u32RegisterValue;
    U32BIT u16Channel;
    ...
    if (OutputBufferLength >= sizeof(U32BIT))
    {
        ...
        u16Channel = pIoctlParams->Channel;
        u32BitOffset = (pIoctlParams->Param1 - DD429_RX_CONTROL_COMMAND_GET__INDEX_START);
...
)
{
    ...
    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_FILTER, "429 - _ARINC429FilterCommand(CH:%ld) - Cmd %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
    ...
    switch (pIoctlParams->Param1)
    {
        ...
        /* ---------------------------------------------------- */
        {
            ...
            *pRdData = _arinc429GetFilter(pDeviceContext, u16Channel, pIoctlParams->Param2);
            break;
        }
        ...
        /* ---------------------------------------------------- */
        {
            ...
            S16BIT *ps16LabelSDI    = NULL;
            U32BIT *pu32Count       = (U32BIT *) pRdData;
            S16BIT *ps16LableSDIOut = (S16BIT *) pIoctlParams->Param4;

            if (OutputBufferLength >= sizeof(*pu32Count))
            {
                ps16LabelSDI = DDC_KERNEL_MALLOC(MAX_NUM_LABEL_SDI * sizeof(S16BIT));
                if (ps16LabelSDI == NULL)
                {
                    break;
                }
                ...
                *pu32Count = 0;
                ...
                for (s16LabelSDI = MIN_LABEL_SDI; s16LabelSDI <= MAX_LABEL_SDI; s16LabelSDI++)
                {
                    ...
                    {
                        ps16LabelSDI[*pu32Count] = s16LabelSDI;

                        (*pu32Count)++;
                    }
                }

                *pBytesReturned = sizeof(*pu32Count);

                /* copy the data to the user buffer */
                result = DDC_COPY_TO_USER((char *)ps16LableSDIOut, (char *)ps16LabelSDI, MAX_NUM_LABEL_SDI * sizeof(S16BIT));
        ...
        {
            /* unknown type */
            DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_FILTER, "429 - _ARINC429FilterCommand(CH:%ld) - Unknown Command %ld !!!\n", pIoctlParams->Channel, pIoctlParams->Param1);
            *pRdData = (U32BIT) ERR_UNKNOWN;
            break;
        }
    }
...
)
{
    ...
    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_MAILBOX, "429 - _ARINC429MailboxCommand(CH:%ld) - Cmd %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
    ...
    switch (pIoctlParams->Param1)
    {
        ...
        /* ---------------------------------------------------- */
        {
            DD429_GET_MAILBOX__OUTPUT_TYPE *psGetMailbox = (DD429_GET_MAILBOX__OUTPUT_TYPE *) pRdData;
            ...
            if (OutputBufferLength >= sizeof(psGetMailbox))
            {
                ...
                U8BIT u8NonNullBufferPointer;

                u32MaxArraySize = pIoctlParams->Param2; /* parameter N passed into the GetMailbox API */
                u8NonNullBufferPointer = (U8BIT) pIoctlParams->Param3;
                ...
                for (i = 0; i < u32MaxMailboxMessages; i++)
                {
                    ...
                    if (((i != 0) && (u32Data & DD429_LABEL_MASK)) ||
                        ((i == 0) && ((u32Data & DD429_LABEL_MASK) == 0))) /* special processing for label 0 */
                    {
                        /* if the user passed in an array size and non-NULL buffer, update the list */
                        if ((u32MaxArraySize > 0) && (u8NonNullBufferPointer))
                        {
                            if (pDeviceContext->u8Arinc429BitFormat == DD429_BITFORMAT_ORIG)
                            {
                                psGetMailbox->su16LabelSDI[psGetMailbox->s16N] = (U16BIT)(u32Data & DD429_LABEL_SDI_MASK);
                            }
                            else /* alternate format */
                            {
                                psGetMailbox->su16LabelSDI[psGetMailbox->s16N] = (U16BIT)((u32Data & DD429_LABEL_MASK) | ((u32Data & DD429_ALT_SDI_MASK) >> DD429_ALT_TO_NORMAL_SDI_OFFSET));
                            }

                            psGetMailbox->s16N++;

                            /* make sure we do not go over the array size */
                            if ((psGetMailbox->s16N >= (S16BIT)u32MaxArraySize) && (u8NonNullBufferPointer))
                            {
                                break;
                            }
                        }
        ...
        /* ---------------------------------------------------- */
        {
            ...
            if (u16LabelSDI <= MAX_LABEL_SDI)
            {
                ...
                if ((u16LabelSDI & DD429_LABEL_MASK) == 0x0000)
                {
                    ...
                    {
                        *pRdData = 0; /* did not receive a new word */
                    }
                    else
                    {
                        *pRdData = 1; /* received a new word */
                    }
                }
            ...
            {
                *pRdData = (U32BIT) ERR_LABELSDI; /* ERROR */
            }
        ...
        /* ---------------------------------------------------- */
        {
            DD429_READ_MAILBOX_IRIG__OUTPUT_TYPE *psReadRxMailboxIrig = (DD429_READ_MAILBOX_IRIG__OUTPUT_TYPE *) pRdData;
        ...
        {
            /* unknown type */
            DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_MAILBOX, "429 - _ARINC429MailboxCommand(CH:%ld) - Unknown Command %ld !!!\n", pIoctlParams->Channel, pIoctlParams->Param1);
            *pRdData = (U32BIT) ERR_UNKNOWN;
            break;
        }
    }
...
)
{
    ...
    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_FIFO, "429 - _ARINC429FifoCommand(CH:%ld) - Cmd %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
    ...
    switch (pIoctlParams->Param1)
    {
        ...
        {
            U32BIT u32ModeValue;
            DD429_GET_RX_QUEUE_STATUS__OUTPUT_TYPE *psRxQueueStatus = (DD429_GET_RX_QUEUE_STATUS__OUTPUT_TYPE *) pRdData;
        ...
        /* ---------------------------------------------------- */
        {
            DD429_READ_DATA_IRIG__OUTPUT_TYPE *psReadRxFifoIrig = (DD429_READ_DATA_IRIG__OUTPUT_TYPE *) pRdData;
        ...
        /* ---------------------------------------------------- */
        {
            ...
            DD429_READ_DATA_IRIG_MORE__OUTPUT_TYPE *psReadRxFifoIrigMore = (DD429_READ_DATA_IRIG_MORE__OUTPUT_TYPE *) pRdData;

            u32MaxNumberWords = pIoctlParams->Param2;

            if (OutputBufferLength >= sizeof(*psReadRxFifoIrigMore))
            {
                *pBytesReturned = sizeof(*psReadRxFifoIrigMore);

                /* default to no messages read */
                ...
                *pBytesReturned = sizeof(*psReadRxFifoIrigMore);
...
#if DDC_TRANSFER_VIA_DMA_429
                {
                    ...
                    else
                    {
                        ...
                        dmaARINC429RxFifoSetup(pDeviceContext,
                            ...
                            (U8BIT *)psReadRxFifoIrigMore->au32Data);
                        ...
                        if (0 == pDeviceContext->pRxChnl429[u16Channel]->u16RxFifoEventCond)
                        {
                            ...
                            nResult = DDC_WAIT_INTERRUPTABLE_TIMEOUT_MS(
                                pDeviceContext->pRxChnl429[u16Channel]->waitqueueARINC429RxFifo,
                                pDeviceContext->pRxChnl429[u16Channel]->u16RxFifoEventCond,
                                500);
                        }
                        else
...
#else /* use direct memory reads */
                {
                    ...
                    if ((u32Tail + u32DataEntriesToRead) > u32MemSize)
                    {
                        ...
                        DDC_BLK_MEM_READ(pDeviceContext, u32RxChannelMemoryBaseAddress + u32Tail,
                            (U32BIT *)&psReadRxFifoIrigMore->au32Data[0], u32DataEntriesToRead_part1, ACEX_32_BIT_ACCESS);

                        DDC_BLK_MEM_READ(pDeviceContext, u32RxChannelMemoryBaseAddress,
                            (U32BIT *)&psReadRxFifoIrigMore->au32Data[u32DataEntriesToRead_part1], u32DataEntriesToRead_part2, ACEX_32_BIT_ACCESS);
                    }
                    else
                    {
                        DDC_BLK_MEM_READ(pDeviceContext, u32RxChannelMemoryBaseAddress + u32Tail,
        ...
        {
            /* unknown type */
            DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_FIFO, "429 - _ARINC429FifoCommand(CH:%ld) - Unknown Command %ld !!!\n", pIoctlParams->Channel, pIoctlParams->Param1);
            *pRdData = (U32BIT) ERR_UNKNOWN;
            break;
        }
    }
...
)
{
    ...
    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_GENERAL, "429 - _ARINC429GeneralCommand(CH:%ld) - Cmd %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
    ...
    switch (pIoctlParams->Param1)
    {
        ...
        /* ---------------------------------------------------- */
        {
            ...
            if (!pDeviceContext->bExtendedScheduler) /*aws*/
            {
            ...
            _ARINC429RxCommand(pDeviceContext, &tempIoctlParams, pRdData, sizeof(*pRdData), &bytesReturned);
            ...
            for (u16Channel = 0; u16Channel < pDeviceContext->u8NumProg429RxTx; u16Channel++)
            {
                ...
                tempIoctlParams.Param2 = DD429_DISABLE;
                _ARINC429FilterCommand(pDeviceContext, &tempIoctlParams, &u32Value, sizeof(&u32Value), &bytesReturned);
        ...
        /* ---------------------------------------------------- */
        {
            U32BIT u32IntEnable = pIoctlParams->Param3;
            ...
            switch (pIoctlParams->Param2)
            {
                ...
                /* --------------------- */
                {
                    ...
                    {
                        ddcUdlBdInterruptClear(pDeviceContext, BD_INT_STATUS_MASK_IRIG_1_SEC);
                    }
        ...
        /* ---------------------------------------------------- */
        {
            ...
            U32BIT *pu32Status = (U32BIT *) pRdData;

            /* NOTE:    This command does not send back an error value. It overwrites the error value location  */
            /*          due to the way the pointer is passed to the driver.                                     */
            ...
            switch (pIoctlParams->Param2)
            {
                ...
                /* --------------------- */
                {
                    ...
                    pu32Status[0] = ARINC429CastGetSerialIOConfig(pDeviceContext, &sIoctlParams);
                    break;
                }
                ...
                /* --------------------- */
                {
                    ...
                    u32BytesRead = sizeof(*pu32Status) * (2 + pDeviceContext->u8NumProg717);

                    if (OutputBufferLength >= u32BytesRead)
                    {
                        ARINC_717_INTERRUPT *pArinc717Interrupt = (ARINC_717_INTERRUPT *) pRdData;
                        U16BIT i;
        ...
        /* ---------------------------------------------------- */
        {
           ...
           /* U32BIT u32MemoryAddress;            */
            ...
            if (status)
            {
                *pRdData = (U32BIT) ERR_429_PROG_RESET;
                break;
            }
            ...
            _ARINC429TxCommand(pDeviceContext, &tempIoctlParams, pRdData, sizeof(*pRdData), &tempBytesReturned);
            ...
            _ARINC429TesterCommand(pDeviceContext, &tempIoctlParams, pRdData, OutputBufferLength, pBytesReturned);
            ...
            tempIoctlParams.Param1 = DD429_RX_COMMAND__RESET_TIMESTAMP;
            _ARINC429RxCommand(pDeviceContext, &tempIoctlParams, pRdData, sizeof(*pRdData), &tempBytesReturned);

            /* Reset Rx filter */
            tempIoctlParams.Param1 = DD429_FILTER_COMMAND__ENABLE;
            tempIoctlParams.Param2 = DD429_DISABLE;
            _ARINC429FilterCommand(pDeviceContext, &tempIoctlParams, pRdData, sizeof(*pRdData), &tempBytesReturned);
        ...
        /* ---------------------------------------------------- */
        {
            DDC_IOCTL_PARAMS *pIoConfigureOutput = (DDC_IOCTL_PARAMS *) pRdData;

            status = ddcUdlBdConfigureIoInterruptConditions(
                pDeviceContext,
                pIoctlParams->Channel,  /* Channel (not used for 429) */
        ...
        {
            /* unknown type */
            DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_GENERAL, "429 - _ARINC429GeneralCommand(CH:%ld) - Unknown Command %ld !!!\n", pIoctlParams->Channel, pIoctlParams->Param1);
...
{
    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_CONTROL, "429 - _ARINC429ControlCommand(CH:%ld) - Cmd %ld, ch %ld\n", pIoctlParams->Channel, pIoctlParams->Param1, pIoctlParams->Param2);
    ...
    switch (pIoctlParams->Param1)
    {
        ...
        /* ---------------------------------------------------- */
        {
            ...
            *pRdData = (U32BIT) SetAioOutput(pDeviceContext, &tempIoctlParams);
        ...
        /* ---------------------------------------------------- */
        {
            ...
            *pRdData = GetAioOutput(pDeviceContext, &tempIoctlParams);
        ...
        /* ---------------------------------------------------- */
        {
            ...
            *pRdData = SetAioDirection(pDeviceContext, &tempIoctlParams);
        ...
        /* ---------------------------------------------------- */
        {
            ...
            *pRdData = GetAioDirection(pDeviceContext, &tempIoctlParams);
        ...
        /* ---------------------------------------------------- */
        {
            ...
            *pRdData = GetAioInput(pDeviceContext, &tempIoctlParams);
        ...
        /* ---------------------------------------------------- */
        {
            *pRdData = GetAioAll(pDeviceContext);
            break;
        }
        ...
        /* ---------------------------------------------------- */
        {
            SetAioAll(pDeviceContext, pIoctlParams->Param2);
            break;
        }
/*************************************************************************************************************/
        ...
        /* ---------------------------------------------------- */
        {
            ...
            /* remap IOCTL parameters */
            tempIoctlCmd.Param1 = pIoctlParams->Param2; /* DIO discrete */
            tempIoctlCmd.Param2 = pIoctlParams->Param3; /* level */

            *pRdData = (U32BIT) SetDioOutput(pDeviceContext, pIoctlParams->Param2, pIoctlParams->Param3);

#if 0
            /* translate the error to a 429 library error value if needed */
            if (*pRdData == ACEX_ERR_AVIONIC)
                *pRdData = (U32BIT)ERR_AVIONIC;
#endif
            break;
        }
        ...
        /* ---------------------------------------------------- */
        {
            ...
            *pRdData = GetDioOutput(pDeviceContext, pIoctlParams->Param2);
        ...
        /* ---------------------------------------------------- */
        {
            ...
            /* remap IOCTL parameters */
            tempIoctlCmd.Param1 = pIoctlParams->Param2; /* DIO discrete */
            tempIoctlCmd.Param2 = pIoctlParams->Param3; /* direction */

            *pRdData = SetDioDirection(pDeviceContext, pIoctlParams->Param2, pIoctlParams->Param3);
        ...
        /* ---------------------------------------------------- */
        {
            ...
            *pRdData = GetDioDirection(pDeviceContext, pIoctlParams->Param2);
        ...
        /* ---------------------------------------------------- */
        {
            ...
            *pRdData = GetDioInput(pDeviceContext, pIoctlParams->Param2);
        ...
        /* ---------------------------------------------------- */
        {
            *pRdData = GetDioAll(pDeviceContext);
            break;
        }
        ...
        {
            /* unknown type */
            DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_CONTROL, "429 - _ARINC429ControlCommand(CH:%ld) - Unknown Command %ld !!!\n", pIoctlParams->Channel, pIoctlParams->Param1);
            *pRdData = (U32BIT) ERR_UNKNOWN;
            break;
        }
    }
...
)
{
    ...
    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_TX, "429 - _ARINC429TxCommand(CH:%ld) - Cmd %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
    ...
    switch (pIoctlParams->Param1)
    {
        ...
        /* ---------------------------------------------------- */
        {
            if (OutputBufferLength >= sizeof(U32BIT))
            {
                ...
                *pRdData = ((u32RegisterValue & DD429_TX_FIFO_MASK__COUNT) >> DD429_TX_FIFO_OFFSET__COUNT);
        ...
        /* ---------------------------------------------------- */
        {
            U32BIT u32TxData = pIoctlParams->Param2;

            *pRdData = (U32BIT) _ARINC429SendAsync(pDeviceContext, u16Channel, u32TxData, DD429_CONTROL_WORD__NO_OPTIONS);
        ...
        /* ---------------------------------------------------- */
        {
            ...
            U32BIT *pu32NumLoaded = (U32BIT *) pRdData;
            U32BIT u32NumItems    = (U32BIT  ) pIoctlParams->Param3;
            U32BIT *pu32DataIn    = (U32BIT *) pIoctlParams->Param4;
            U32BIT *pu32Data      = NULL;

            *pu32NumLoaded = 0;

            if (OutputBufferLength >= sizeof(*pu32NumLoaded))
            {
                pu32Data = DDC_KERNEL_MALLOC(u32NumItems * sizeof(U32BIT));
                if (pu32Data == NULL)
                {
                    break;
                }

                result = DDC_COPY_FROM_USER((char *)pu32Data, (char *)pu32DataIn, u32NumItems * sizeof(U32BIT));
                if (result != 0)
                {
                    break;
                }
                ...
                DDC_KERNEL_FREE(pu32Data);

                *pBytesReturned = sizeof(*pu32NumLoaded);
            }
            else
            {
                status = DDC_UDL_ERROR__BUFFER_SIZE;
        ...
        /* ---------------------------------------------------- */
        {
            ...
            else
            {
                ...
                if (u32RegisterValue == DD429_ACTIVE)
                {
                    SCHEDULER_DATA SchedulerData;
                    SchedulerData.u32Channel = u16Channel;
                    SchedulerData.u16Index = pIoctlParams->Param2;
                    SchedulerData.u32Data = 0x00000000;
                    SchedulerData.u32Control = 0x00000000;
                    SchedulerData.u16Frequency = 0x0000;
                    SchedulerData.u16Offset = 0x0000;
        ...
        /* ---------------------------------------------------- */
        {
            DD429_GET_ALL_REPEATED__OUTPUT_TYPE *psAllRepeated = (DD429_GET_ALL_REPEATED__OUTPUT_TYPE *) pRdData;
            ...
            if (OutputBufferLength >= sizeof(*psAllRepeated))
            {
                ...
                else
                {
                    ...
                    _ARINC429TesterCommand(pDeviceContext, pIoctlParams, pRdData, OutputBufferLength, pBytesReturned);
                }
        ...
        {
            /* unknown type */
            DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_TX, "429 - _ARINC429TxCommand(CH:%ld) - Unknown Command %ld !!!\n", pIoctlParams->Channel, pIoctlParams->Param1);
            *pRdData = (U32BIT) ERR_UNKNOWN;
            break;
        }
    }
...
)
{
    ...
    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_TESTER,"429 - _ARINC429TesterCommand(CH:%ld) - Cmd %ld\n", pIoctlParams->Channel, pIoctlParams->Param1);
    ...
    switch (pIoctlParams->Param1)
    {
        ...
        /* ---------------------------------------------------- */
        {
            ...
            U32BIT u32TesterOptions;
            S16BIT s16Match;
            U16BIT u16LabelSDI;
            U16BIT u16QueueIndex;
            U16BIT u16DataIndex;
            
            U32BIT u32Data        = (U32BIT  )pIoctlParams->Param2;
            U32BIT *pu32OptionIn  = (U32BIT *)pIoctlParams->Param3;
            U16BIT u16Frequency   = (U16BIT  )pIoctlParams->Param4;
            U16BIT u16Offset      = (U16BIT  )pIoctlParams->Param5;

            S16BIT s16RepeatCount = 0;

            /* copy tester structure */
            result = DDC_COPY_FROM_USER((char *)&sTesterOptions, (char *)pu32OptionIn, sizeof(DD429_TESTER_OPTIONS_TYPE));
            if (result != 0)
            {
                break;
            }
            ...
            if (pDeviceContext->u8Arinc429BitFormat == DD429_BITFORMAT_ORIG)
            {
                u16LabelSDI = (U16BIT)(u32Data & DD429_LABEL_SDI_MASK);
            }
            else
            {
                u16LabelSDI = (U16BIT)(((u32Data & DD429_ALT_SDI_MASK) >> DD429_ALT_TO_NORMAL_SDI_OFFSET) || (u32Data & DD429_LABEL_MASK));
            }
            ...
            if (!pDeviceContext->bExtendedScheduler)
            {
                ...
                for (u16DataIndex = 0; u16DataIndex < DD429_TX_SCHED_MESSAGE_MEM_DEPTH; u16DataIndex++)
                {
                    ...
                    if (s16Match)
                    {
                        ...
                        for (u16QueueIndex = 0; u16QueueIndex < DD429_TX_SCHED_QUEUE_DEPTH; u16QueueIndex++)
                        {
                            ...
                            if ((u32DataPointer == (U32BIT)u16DataIndex) && (u32QueueChannel == (U32BIT)u16Channel))
                            {
                                DD429_MEM_WRITE_TX_GLOBAL(pDeviceContext, (DD429_TX_SCHED_MSG_DATA_OFFSET + u16DataIndex), u32Data);

                                u32TesterOptions = _ConvertTesterOptionsToControlWord(&sTesterOptions);
                                DD429_MEM_WRITE_TX_GLOBAL(pDeviceContext, (DD429_TX_SCHED_MSG_CONTROL_OFFSET + u16DataIndex), u32TesterOptions);

                                DD429_MEM_WRITE_TX_GLOBAL(pDeviceContext, (DD429_TX_SCHED_QUEUE_FREQ_OFFSET + u16QueueIndex), u16Frequency);
                                ...
                                if (pDeviceContext->U.sTxScheduler.u16Offset[u16QueueIndex] != u16Offset)
                                {
                                    DD429_MEM_WRITE_TX_GLOBAL(pDeviceContext, (DD429_TX_SCHED_QUEUE_OFFSET_TIME_OFFSET + u16QueueIndex), u16Offset);

                                    pDeviceContext->U.sTxScheduler.u16Offset[u16QueueIndex] = u16Offset;
                                }

                                /* Update Tx Scheduler Table for data modification function (0 base) */
                                pDeviceContext->U.sTxScheduler.u16Channel[u16QueueIndex] = u16Channel + 1;
                ...
                /* Update Tx Scheduler Table for data modification function (0 base) */
                pDeviceContext->U.sTxScheduler.u16Offset[u16QueueIndex]  = u16Offset; /* Tx offset */
                pDeviceContext->U.sTxScheduler.u16Channel[u16QueueIndex] = u16Channel + 1;
            ...
            {
                /* we parsed the labelsdi which we use as the index */
                pIoctlParams->Param1 = DD429_TESTER_COMMAND__ADD_REPEATED_ITEM;
                pIoctlParams->Param6 = u16LabelSDI;
        ...
        /* ---------------------------------------------------- */
        {
            ...
            U16BIT u16Frequency   = (U16BIT  )pIoctlParams->Param4;
            U16BIT u16Offset      = (U16BIT  )pIoctlParams->Param5;
            U16BIT u16ItemIndex   = (U16BIT  )pIoctlParams->Param6;
            ...
            SchedulerData.u32Channel   = u16Channel;
            SchedulerData.u16Index     = u16ItemIndex;
            SchedulerData.u32Data      = u32Data;
            SchedulerData.u32Control   = _ConvertTesterOptionsToControlWord(&sTesterOptions);
            SchedulerData.u16Frequency = u16Frequency;
            SchedulerData.u16Offset    = u16Offset;
            SchedulerData.u8Active     = DD429_ACTIVE;

            *pRdData = (U32BIT) ERR_SUCCESS;
            u32RegisterAddress = *(pDeviceContext->sTxChnlExtend429[u16Channel].pu32MemBA) + u16ItemIndex;
            DDC_MEM_READ(pDeviceContext, (u32RegisterAddress), &u32RegisterValue, ACEX_32_BIT_ACCESS);
            if (u32RegisterValue == DD429_ACTIVE)
            {
                ...
                _ARINC429TxCommand(pDeviceContext, &tempIoctlCmd, pRdData, sizeof(*pRdData), &tempBytesReturned);
        ...
        /* ---------------------------------------------------- */
        {
            DD429_GET_REPEATED__OUTPUT_TYPE *psGetRepeatedOutput = (DD429_GET_REPEATED__OUTPUT_TYPE *) pRdData;
            ...
            if (OutputBufferLength >= sizeof(*psGetRepeatedOutput))
            {
                ...
                U32BIT u32DataLabelSDI = 0;
                U32BIT u32DataSDI = 0;
                U32BIT u32RequestedLabelSDI = pIoctlParams->Param2;
                U16BIT u16QueueIndex = 0;
                U8BIT bMatch = FALSE;
        ...
        /* ---------------------------------------------------- */
        {
            /*printf("GET ALL RPT ITEM\n");*/
            DD429_GET_ALL_REPEATED__OUTPUT_TYPE *psAllRepeated = (DD429_GET_ALL_REPEATED__OUTPUT_TYPE *) pRdData;

            if (OutputBufferLength >= sizeof(*psAllRepeated))
            {
                U16BIT BufferSize = pIoctlParams->Param2;
                U16BIT u16QueueIndex;
        ...
        /* ---------------------------------------------------- */
        {
            ...
            U8BIT u8SDI;
            U8BIT u8Label;
            U32BIT u32ModifiedControl;

            S16BIT s16Result     = ERR_FUNCTION;
            U16BIT u16QueueIndex = 0;

            U32BIT u32ModifiedData    = (U32BIT  )pIoctlParams->Param2;
            U32BIT *pu32OptionIn      = (U32BIT *)pIoctlParams->Param3;
            U32BIT u32ModifiedOption  = (U32BIT  )pIoctlParams->Param4;
            ...
            u32ModifiedControl = _ConvertTesterOptionsToControlWord(&sTesterOptions);
            ...
            else
            {
                ...
                {
                    for (u16QueueIndex = u8Label; u16QueueIndex < DD429_TX_SCHED_MESSAGE_MEM_DEPTH; u16QueueIndex+= 256)
                    {
        ...
        /* ---------------------------------------------------- */
        {
            ...
            U32BIT u32Data       = (U32BIT  )pIoctlParams->Param2;
            U32BIT *pu32OptionIn = (U32BIT *)pIoctlParams->Param3;
            U16BIT u16ItemIndex  = (U16BIT  )pIoctlParams->Param4;
        ...
        /* ---------------------------------------------------- */
        {
            U32BIT u32Amplitude = pIoctlParams->Param2;
                    ...
                    ((pDeviceContext->pBoardInfo[ACEX_BD_INSTANCE_0]->sBdReg.u32ModelNum == 0x06) && (u16Channel >= 6))))
            {
                ...
                *pRdData = (U32BIT) ERR_FEATURE_NOT_SUPPORTED_CH;
                break;
            }
        ...
        /* ---------------------------------------------------- */
        {
            ...
            U32BIT u32Data       = (U32BIT  ) pIoctlParams->Param4;
            U32BIT *pu32OptionIn = (U32BIT *) pIoctlParams->Param5;
            ...
            u32ControlWord = _ConvertTesterOptionsToControlWord(&sTesterOptions);
            *pRdData = (U32BIT) _ARINC429SendAsync(pDeviceContext, u16Channel, u32Data, u32ControlWord);
        ...
        /* ---------------------------------------------------- */
        {
            ...
            DD429_TESTER_OPTIONS_TYPE *psTesterOptions = NULL;
            U32BIT *pu32Data      = NULL;
            U32BIT *pu32NumLoaded = (U32BIT *) pRdData;
            ...
            if (OutputBufferLength >= sizeof(*pu32NumLoaded))
            {
                ...
                psTesterOptions = DDC_KERNEL_MALLOC(u32NumItems * sizeof(DD429_TESTER_OPTIONS_TYPE));
                if (psTesterOptions == NULL)
                {
                    break;
                }
                ...
                /* copy tester structure */
                result = DDC_COPY_FROM_USER((char *)psTesterOptions, (char *)pu32OptionIn, u32NumItems * sizeof(DD429_TESTER_OPTIONS_TYPE));
                if (result != 0)
                {
                    break;
                }
                ...
                *pu32NumLoaded = 0;
                ...
                DDC_KERNEL_FREE(pu32Data);
                DDC_KERNEL_FREE(psTesterOptions);
        ...
        /* ---------------------------------------------------- */
        {
            U32BIT u32Speed = pIoctlParams->Param2;
            U32BIT u32TableIndex;
        ...
        /* ---------------------------------------------------- */
        {
            ...
            DDC_REG_READ(pDeviceContext, u32RegisterAddress, pRdData);

            *pRdData = _ConvertTableIndexToSpeed(*pRdData);
            *pBytesReturned = sizeof(U32BIT);
            break;
        }
        ...
        /* ---------------------------------------------------- */
        {
            ...
            u32Resolution = pIoctlParams->Param2;
        ...
        /* ---------------------------------------------------- */
        {
            DD429_GET_TX_SCHEDULE_RESOLUTION__OUTPUT_TYPE *psGetTxFrameResolution = (DD429_GET_TX_SCHEDULE_RESOLUTION__OUTPUT_TYPE *) pRdData;
            U32BIT u32Resolution;
        ...
        /* ---------------------------------------------------- */
        {
            ...
            U32BIT u32Control      = (U32BIT  ) pIoctlParams->Param2;
            U32BIT u32PayloadCount = (U32BIT  ) pIoctlParams->Param3;
            U32BIT *pu32TxTableIn  = (U32BIT *) pIoctlParams->Param4;
            ...
            switch (u32Control)
            {
                ...
                {
                    pu32TxTable = DDC_KERNEL_MALLOC((u32PayloadCount << 2)); /* convert # of 32-bit entries t*/
                    if (pu32TxTable == NULL)
                    {
                        break;
                    }

                    result = DDC_COPY_FROM_USER((char *)pu32TxTable, (char *)pu32TxTableIn, (u32PayloadCount << 2));
                    if (result != 0)
                    {
                        break;
                    }
...
#if DDC_TRANSFER_VIA_DMA_429
                    {
                        ...
                        if (0 == pDeviceContext->pTxChnl429[u16Channel]->u16ARINC429SetTxFrameControlEventCond)
                        {
                            ...
                            nResult = DDC_WAIT_INTERRUPTABLE_TIMEOUT_MS(
                                pDeviceContext->pTxChnl429[u16Channel]->waitqueueARINC429SetTxFrameControl,
                                pDeviceContext->pTxChnl429[u16Channel]->u16ARINC429SetTxFrameControlEventCond,
                                500);
                        }
                        else
...
#else /* do not use DDC_TRANSFER_VIA_DMA_429 */
                    {
                        unsigned int i;
                ...
                default:
                {
                    *pRdData = (U32BIT) ERR_FRAME_CONTROL_TYPE;
                    break;
                }
            }
        ...
        /* ---------------------------------------------------- */
        {
            ...
            U32BIT u32Priority   = (U32BIT  ) pIoctlParams->Param2;
            U32BIT u32Data       = (U32BIT  ) pIoctlParams->Param4;
            U32BIT *pu32OptionIn = (U32BIT *) pIoctlParams->Param5;
        ...
        /* ---------------------------------------------------- */
        {
            ...
            U32BIT result;
            S16BIT *ps16Channels;
            U16BIT u16NumChannels  = (U16BIT  ) pIoctlParams->Param2;
            S16BIT *ps16ChannelsIn = (S16BIT *) pIoctlParams->Param4;

            ps16Channels = DDC_KERNEL_MALLOC(u16NumChannels * sizeof(S16BIT)); /* convert # of 32-bit entries t*/
            if (ps16Channels == NULL)
            {
                break;
            }

            result = DDC_COPY_FROM_USER((char *)ps16Channels, (char *)ps16ChannelsIn, u16NumChannels * sizeof(S16BIT));
            if (result != 0)
            {
                break;
            }
            ...
            for (u32Index = 0; u32Index < (U32BIT)u16NumChannels; u32Index++)
            {
                u32RegisterValue |= 1 << (ps16Channels[u32Index] - 1); /* minus 1 to convert to 0 based index */
            }

            if (*pRdData == DDC_UDL_ERROR__SUCCESS)
            {
...
#if DDC_TRANSFER_VIA_DMA_429
            {
                if (pDeviceContext->pBoardInfo[ACEX_BD_INSTANCE_0]->sBdVoltMonX8.pu32RegBA)
                {
                    ...
                    if (0 == pDeviceContext->u16Arinc429VoltageMonitoringEventCond)
                    {
                        ...
                        nResult = DDC_WAIT_INTERRUPTABLE_TIMEOUT_MS(
                            pDeviceContext->waitqueueArinc429VoltageMonitoring,
                            pDeviceContext->u16Arinc429VoltageMonitoringEventCond,
                            500);
                    }
                    else
        ...
        /* ---------------------------------------------------- */
        {
            *pRdData = ARINC429CastSerialIORegRead(pDeviceContext, pIoctlParams);
            break;
        }
        ...
        /* ---------------------------------------------------- */
        {
            *pRdData = ARINC429CastGetSerialIOConfig(pDeviceContext, pIoctlParams);
            break;
        }
        ...
        /* ---------------------------------------------------- */
        {
            U16BIT u16LabelSdi = pIoctlParams->Param2;
            U32BIT u32Mode = pIoctlParams->Param3;
            U32BIT u32Value = pIoctlParams->Param4;
            U16BIT u16LabelLoop = 0;

            U16BIT u16LabelOnly = (u32Mode & REPEATER_OPTION__LABEL_ONLY) ? REPEATER_LABEL_ONLY_HW : REPEATER_LABEL_SDI_HW;

            /* disable the previous mode before writing the new way */
            u32RegisterAddress = *(pDeviceContext->pTxChnl429[u16Channel]->pu32MemBA) + (pIoctlParams->Param2 + DD429_REPEATER_MODE_OFFSET);
            DDC_MEM_READ(pDeviceContext, (u32RegisterAddress), &u32RegisterValue, ACEX_32_BIT_ACCESS);
            if (u32RegisterValue & REPEATER_OPTION__LABEL_ONLY)
            {
                u32RegisterValue = u32Mode + REPEATER_DISABLED_HW + u16LabelOnly;
                for (u16LabelLoop = u16LabelSdi & DD429_LABEL_MASK; u16LabelLoop <= MAX_NUM_LABEL_SDI - 1; u16LabelLoop+=256)
                {
                    u32RegisterAddress = *(pDeviceContext->pTxChnl429[u16Channel]->pu32MemBA) + (u16LabelLoop & DD429_LABEL_SDI_MASK) + DD429_REPEATER_MODE_OFFSET;
                    DDC_MEM_WRITE(pDeviceContext, (u32RegisterAddress), &u32RegisterValue, ACEX_32_BIT_ACCESS);

                    u32RegisterValue = 0;
                    u32RegisterAddress = *(pDeviceContext->pTxChnl429[u16Channel]->pu32MemBA) + (u16LabelLoop & DD429_LABEL_SDI_MASK);
                    DDC_MEM_WRITE(pDeviceContext, (u32RegisterAddress), &u32RegisterValue, ACEX_32_BIT_ACCESS);
                }
            }
            else
            {
                u32RegisterAddress = u32Mode + REPEATER_DISABLED_HW + u16LabelOnly;
                u32RegisterAddress = *(pDeviceContext->pTxChnl429[u16Channel]->pu32MemBA) + (u16LabelSdi & DD429_LABEL_SDI_MASK) + DD429_REPEATER_MODE_OFFSET;
                DDC_MEM_WRITE(pDeviceContext, (u32RegisterAddress), &u32RegisterValue, ACEX_32_BIT_ACCESS);

                u32RegisterValue = 0;
                u32RegisterAddress = *(pDeviceContext->pTxChnl429[u16Channel]->pu32MemBA) + (u16LabelSdi & DD429_LABEL_SDI_MASK);
                DDC_MEM_WRITE(pDeviceContext, (u32RegisterAddress), &u32RegisterValue, ACEX_32_BIT_ACCESS);
            }


            /* parse SW mode to HW mode */
            /* write mode last because it makes the label/sdi active */
            switch (u32Mode & REPEATER_MODE_MASK_SW)
            {
                case REPEATER_MODE__DISABLED:
                    u32Mode += REPEATER_DISABLED_HW + u16LabelOnly;
                    u32Value = 0;
                    if (u16LabelOnly == REPEATER_LABEL_ONLY_HW)
                    {
                        for (u16LabelLoop = u16LabelSdi & DD429_LABEL_MASK; u16LabelLoop <= MAX_NUM_LABEL_SDI - 1; u16LabelLoop+=256)
                        {
                            u32RegisterAddress = *(pDeviceContext->pTxChnl429[u16Channel]->pu32MemBA) + (u16LabelLoop & DD429_LABEL_SDI_MASK);
                            DDC_MEM_WRITE(pDeviceContext, (u32RegisterAddress), &u32Value, ACEX_32_BIT_ACCESS);

                            u32RegisterAddress = *(pDeviceContext->pTxChnl429[u16Channel]->pu32MemBA) + ((u16LabelLoop & DD429_LABEL_SDI_MASK) + DD429_REPEATER_MODE_OFFSET);
                            DDC_MEM_WRITE(pDeviceContext, (u32RegisterAddress), &u32Mode, ACEX_32_BIT_ACCESS);
                        }
                    }
                    else if (u16LabelOnly == REPEATER_LABEL_SDI_HW)
                    {
                        u32RegisterAddress = *(pDeviceContext->pTxChnl429[u16Channel]->pu32MemBA) + (u16LabelSdi & DD429_LABEL_SDI_MASK);
                        DDC_MEM_WRITE(pDeviceContext, (u32RegisterAddress), &u32Value, ACEX_32_BIT_ACCESS);
                ...
                case REPEATER_MODE__REPEAT:
                    u32Mode += REPEATER_REPEAT_HW + u16LabelOnly;
                    u32Value = 0;
                    if (u16LabelOnly == REPEATER_LABEL_ONLY_HW)
                    {
                        for (u16LabelLoop = u16LabelSdi & DD429_LABEL_MASK; u16LabelLoop <= MAX_NUM_LABEL_SDI - 1; u16LabelLoop+=256)
                ...
                case REPEATER_MODE__DATA_REPLACE:
                    u32Mode += REPEATER_DATA_REPLACE_HW + u16LabelOnly;
                    if (u16LabelOnly == REPEATER_LABEL_ONLY_HW)
                    {
                        u32Value <<= REPEATER_LABEL_DATA_SHIFT;
                        for (u16LabelLoop = u16LabelSdi & DD429_LABEL_MASK; u16LabelLoop <= MAX_NUM_LABEL_SDI - 1; u16LabelLoop+=256)
                        {
                            u32RegisterAddress = *(pDeviceContext->pTxChnl429[u16Channel]->pu32MemBA) + (u16LabelLoop & DD429_LABEL_SDI_MASK);
                            DDC_MEM_WRITE(pDeviceContext, (u32RegisterAddress), &u32Value, ACEX_32_BIT_ACCESS);
                    ...
                    {
                        u32Value <<= REPEATER_LABELSDI_DATA_SHIFT;
                        u32RegisterAddress = *(pDeviceContext->pTxChnl429[u16Channel]->pu32MemBA) + (u16LabelSdi & DD429_LABEL_SDI_MASK);
                        DDC_MEM_WRITE(pDeviceContext, (u32RegisterAddress), &u32Value, ACEX_32_BIT_ACCESS);
                ...
                case REPEATER_MODE__LABEL_REPLACE:
                    u32Mode += REPEATER_LABEL_REPLACE_HW + u16LabelOnly;
                    if (u16LabelOnly == REPEATER_LABEL_ONLY_HW)
                    {
                        u32Value &= DD429_LABEL_MASK;
                        for (u16LabelLoop = u16LabelSdi & DD429_LABEL_MASK; u16LabelLoop <= MAX_NUM_LABEL_SDI - 1; u16LabelLoop+=256)
                        {
                            u32RegisterAddress = *(pDeviceContext->pTxChnl429[u16Channel]->pu32MemBA) + (u16LabelLoop & DD429_LABEL_SDI_MASK);
                            DDC_MEM_WRITE(pDeviceContext, (u32RegisterAddress), &u32Value, ACEX_32_BIT_ACCESS);
                ...
                case REPEATER_MODE__MESSAGE_REPLACE:
                    u32Mode += REPEATER_MESSAGE_REPLACE_HW + u16LabelOnly;
                    if (u16LabelOnly == REPEATER_LABEL_ONLY_HW)
                    {
                        for (u16LabelLoop = u16LabelSdi & DD429_LABEL_MASK; u16LabelLoop <= MAX_NUM_LABEL_SDI - 1; u16LabelLoop+=256)
                        {
                ...
                case REPEATER_MODE__DATA_FLIP:
                    u32Mode += REPEATER_MESSAGE_FLIP_HW + u16LabelOnly;
                    if (u16LabelOnly == REPEATER_LABEL_ONLY_HW)
                    {
                        u32Value <<= REPEATER_LABEL_DATA_SHIFT;
                        for (u16LabelLoop = u16LabelSdi & DD429_LABEL_MASK; u16LabelLoop <= MAX_NUM_LABEL_SDI - 1; u16LabelLoop+=256)
                ...
                case REPEATER_MODE__LABEL_FLIP:
                    u32Mode += REPEATER_MESSAGE_FLIP_HW + u16LabelOnly;
                    if (u16LabelOnly == REPEATER_LABEL_ONLY_HW)
                    {
                        u32Value &= DD429_LABEL_MASK;
                ...
                case REPEATER_MODE__MESSAGE_FLIP:
                    u32Mode += REPEATER_MESSAGE_FLIP_HW + u16LabelOnly;
                    if (u16LabelOnly == REPEATER_LABEL_ONLY_HW)
                    {
                        for (u16LabelLoop = u16LabelSdi & DD429_LABEL_MASK; u16LabelLoop <= MAX_NUM_LABEL_SDI - 1; u16LabelLoop+=256)
                ...
                case REPEATER_MODE__DATA_SET:
                    u32Mode += REPEATER_MESSAGE_SET_HW + u16LabelOnly;
                    if (u16LabelOnly == REPEATER_LABEL_ONLY_HW)
                    {
                        u32Value <<= REPEATER_LABEL_DATA_SHIFT;
                        for (u16LabelLoop = u16LabelSdi & DD429_LABEL_MASK; u16LabelLoop <= MAX_NUM_LABEL_SDI - 1; u16LabelLoop+=256)
                ...
                case REPEATER_MODE__MESSAGE_SET:
                    u32Mode += REPEATER_MESSAGE_SET_HW + u16LabelOnly;
                    if (u16LabelOnly == REPEATER_LABEL_ONLY_HW)
                    {
                        for (u16LabelLoop = u16LabelSdi & DD429_LABEL_MASK; u16LabelLoop <= MAX_NUM_LABEL_SDI - 1; u16LabelLoop+=256)
                ...
                case REPEATER_MODE__DATA_CLEAR:
                    u32Mode += REPEATER_MESSAGE_CLEAR_HW + u16LabelOnly;
                    if (u16LabelOnly == REPEATER_LABEL_ONLY_HW)
                    {
                        u32Value <<= REPEATER_LABEL_DATA_SHIFT;
                        for (u16LabelLoop = u16LabelSdi & DD429_LABEL_MASK; u16LabelLoop <= MAX_NUM_LABEL_SDI - 1; u16LabelLoop+=256)
                ...
                case REPEATER_MODE__LABEL_CLEAR:
                    u32Mode += REPEATER_MESSAGE_CLEAR_HW + u16LabelOnly;
                    if (u16LabelOnly == REPEATER_LABEL_ONLY_HW)
                    {
                        u32Value &= DD429_LABEL_MASK;
                ...
                case REPEATER_MODE__MESSAGE_CLEAR:
                    u32Mode += REPEATER_MESSAGE_CLEAR_HW + u16LabelOnly;
                    if (u16LabelOnly == REPEATER_LABEL_ONLY_HW)
                    {
                        for (u16LabelLoop = u16LabelSdi & DD429_LABEL_MASK; u16LabelLoop <= MAX_NUM_LABEL_SDI - 1; u16LabelLoop+=256)
        ...
        /* ---------------------------------------------------- */
        {
            DDC_IOCTL_PARAMS *pReturnIoCtlCmd = (DDC_IOCTL_PARAMS *) pRdData;

            if (OutputBufferLength >= sizeof(*pReturnIoCtlCmd))
            {
                u32RegisterAddress = *(pDeviceContext->pTxChnl429[u16Channel]->pu32MemBA) + (pIoctlParams->Param2);
                DDC_MEM_READ(pDeviceContext, (u32RegisterAddress), (U32BIT *)&pReturnIoCtlCmd->Param2, ACEX_32_BIT_ACCESS);

                u32RegisterAddress = *(pDeviceContext->pTxChnl429[u16Channel]->pu32MemBA) + (pIoctlParams->Param2 + DD429_REPEATER_MODE_OFFSET);
                DDC_MEM_READ(pDeviceContext, (u32RegisterAddress), (U32BIT *)&pReturnIoCtlCmd->Param3, ACEX_32_BIT_ACCESS);

                pReturnIoCtlCmd->Param1 = ERR_SUCCESS;

                *pBytesReturned = sizeof(*pReturnIoCtlCmd);
            }
            else
            {
                status = DDC_UDL_ERROR__BUFFER_SIZE;
        ...
        {
            /* unknown type */
            DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_COMMAND_TESTER, "429 - _ARINC429TesterCommand(CH:%ld) - Unknown Command %ld !!!\n", pIoctlParams->Channel, pIoctlParams->Param1);
            *pRdData = (U32BIT) ERR_UNKNOWN;
            break;
        }
    }
...
)
{
    ...
    *pReadData = DDC_UDL_ERROR__SUCCESS;
    *pBytesReturned = sizeof(U32BIT);
    ...
    switch (pIoctlParams->Param1)
    {
        ...
        {
            *pBytesReturned = ARINC429ReadRxHostBuffer(pDeviceContext, pIoctlParams->Channel, pIoctlParams->Param2, pReadData);
            break;
        }
...
)
{
    ...
    if (sHbuf->pu8Buffer == NULL)
    {
        ...
        sHbuf->pu8Buffer = (U8BIT *) DDC_KERNEL_VIRTUAL_MALLOC(u32BufferByteSize);
        if (sHbuf->pu8Buffer == NULL)
        {
...
)
{
    ...
    if (sHbuf->pu8Buffer != NULL)
    {
        ...
        DDC_KERNEL_VIRTUAL_FREE(sHbuf->pu8Buffer);
...
/* //todo    DDC_REG_READ(pDeviceContext, u32RegisterAddress, &u32RegisterValue, DDC_USB_ACC_TYPE_WAIT); */
    ...
    *pReadData = u32RegisterValue;
    ...
    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC429, DDC_DBG_ARINC429_GETRXQUEUESTATUS, "429 - ARINC429GetQueueStatus: Channel: %02d status: 0x%08X\n",
        u16Channel, u32RegisterValue);
...
)
{
    ...
    if (pDeviceContext->bArincSerialIsrEnabled)
    {
        ...
        status = irqDisableInterrupt429(pDeviceContext, u8CardNumber);
...
/*-------------------------------------------------------------------------------
   ...
   Parameters:
      ...
      In  pRdData           - list of messages to be transmitted.
...
)
{
    ...
    for (i = 0, j = 0; j < (OutputBufferLength); i+=2, j++)
    {
        *(pDeviceContext->p429TxMemory +i) = u32Address;
        *(pDeviceContext->p429TxMemory + i + 1) = *(pRdData + j);
...
)
{
    ...
    *(pDeviceContext->p429TxMemory + 3) = pIoctlParams->Param3;
