...
)
{
    ...
    if (pTrigger->pu32RegBA)
    {
        ...
        pTrigger->u32Head = 0;
        pTrigger->u32Tail = 0;
        pTrigger->hBuf = DDC_KERNEL_MALLOC(pTrigger->u32StatusDwords << 2);
        if (!pTrigger->hBuf)
        {
...
)
{
    ...
    if (pIrqBuf)
    {
        ...
        if (pTrigger->u32Count < pTrigger->u32StatusEntries)
        {
            ...
            dbg_print(DBG_MODULE_MF, DBG_PRIORITY_INFO, "%d: Trg status %08X\n", pTrigger->u32Tail, *(pIrqBuf + pTrigger->u32Tail));
            ...
            dbg_print(DBG_MODULE_MF, DBG_PRIORITY_INFO, "%d: Interrupt status %08X\n\n", pTrigger->u32Tail, *(pIrqBuf + pTrigger->u32Tail));
    ...
    if (pIrqBuf)
    {
        ...
        if (pTrigger->u32Count)
        {
            ...
            *(pRdData + 1) = *(pIrqBuf + pTrigger->u32Head);
            dbg_print(DBG_MODULE_MF, DBG_PRIORITY_INFO, "%d: Trg IRQ    %08X\n", pTrigger->u32Head, *(pRdData + 1));
