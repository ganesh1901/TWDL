...
#define RT_DMA  1 /* 1- use DMA for RT, 0 - use DDC_BLK_MEM_READ */
...
)
{
    ...
    U32BIT u32Data;
#if (RT_DMA)
    U32BIT u32AbsolutStartAddr;
#endif

    U8BIT *pBuf = NULL;


    dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_FUNC, "ENTER->Channel:%d\n", u8Ch);
    ...
    dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO, "**** OUTFIFO STATUS ****  NumEntries: %d \n", u32NumEntries);

    if (u32Desc[0] & IMP_OUTFIFO_STATUS_FULL)
    {
        dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO, " OFIFO FULL\n");
    ...
    for (j = 0; j < u32NumEntries; j++)
    {
        ...
        if (u32NumCmds > 0)
        {
            dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO, "Channel:%d **** OUT DQ LO/HI **** MemAddr:%08X   NumCmds:%d / NumWds:%d\n", \
        ...
        switch (u32HostID)
        {
            case RT_HBUF_IMP_ID:
            {
                dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO, "Ch%d: RT_HBUF_IMP_ID\n", u8Ch);
                ...
                dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO, "Channel:%d ImpCmd Complete\n", u8Ch);
                ...
                if (u32NumCmds > 0)
                {
                    ...
                    pBuf = pRT->sHbuf.pu8hbufMemory + pRT->sHbuf.u32HbufLstWr;          /* the destination buffer pointer     */
...
#if (!RT_DMA) /* move to DMA complete function if DMA */
            ...
            {
                dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO, "Ch%d: RT_DATA_TFR_IMP_ID\n", u8Ch);
            ...
            default:
            {
                ...
                dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO, "Invalid HostId %08x\n", u32HostID);
                break;
            }
        }
    }
    ...
    dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_FUNC, "WRITE IMP TRIG REG:%08x VALUE:%08x (IMP TRIGGER)\n"
                                                 "Exit impInterruptHandler\n",
        ((*(pCh->sImpRT.pu32RegBA)) + ACEX_1553_IMP_REG_IMP_BLK_TRIG), u32Data);

    dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_FUNC, "END->\n");
}
...
)
{
    ...
    dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_FUNC, "ENTER->\n");
...
)
{
    ...
    dbg_print(DBG_MODULE_IMP,
        DBG_PRIORITY_INFO_REG, "WRITE IMP REG:%08x VALUE:%08x (IMP INT MASK)\n", ((*(pCh->sImpRT.pu32RegBA)) + ACEX_1553_IMP_REG_INT_EN_MASK), u32Data);
    ...
    dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO_REG, "WRITE IMP INPUT Q: LO:%08x HI:%08x Trigger:%08x\n", u32Desc[0], u32Desc[1], u32Desc[2]);
}
...
)
{
    ...
    dbg_print(DBG_MODULE_IMP,
        DBG_PRIORITY_INFO, "WRITE BD RESET REG:%d VALUE:0x%08x (RESET IMPROVEMENTS)\n", REG_BD_RESET_COMPONENT_SF, u32Data);     /* ??, by BZ */
    ...
    dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO, "WRITE ImpBc REG:%08x VALUE:%08x (IMP MEM BaseOffset)\n", u32Register, u32Data);
    ...
    dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO, "WRITE ImpBc REG:%08x VALUE:%08x (IMP MEM SIZE)\n", u32Register, u32Data);
        ...
        (pCh->sImpBC.u32ComponentType != UM_COMPONENTS_ID_MIL_STD_1553_MF_IMP))
    {
        ...
        dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO, "WRITE ImpRT REG:%08x VALUE:%08x (IMP MEM BaseOffset)\n", u32Register, u32Data);
        ...
        dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO, "WRITE ImpRT REG:%08x VALUE:%08x (IMP MEM SIZE)\n", u32Register, u32Data);
    }
    else
    {
...
)
{
    ...
    dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO_REG, "WRITE ImpRT REG (IMP NUM QUE TFRS):%08x VALUE:%08x\n", (int)u32Register, (int)u32Data);
    ...
    dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO_REG, "WRITE ImpRT REG:%08x VALUE:%08x \n",
...
)
{
    ...
    dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO, "CMDSTKPTR:%08X IMPPTR:%08X\n", (int)u32CurrentCmdStkPtr, (int)u32Data);
    ...
    {
        dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO, "POINTERS NOT THE SAME\n");
    ...
    {
        dbg_print(DBG_MODULE_IMP, DBG_PRIORITY_INFO, "POINTERS SAME\n");
    }
