...
)
{
    ...
    {
        pDeviceContext->pChannel1553[i] = (struct _ACEX_1553_CHANNEL_TYPE *) DDC_KERNEL_MALLOC(sizeof(*pDeviceContext->pChannel1553[i]));
...
)
{
    ...
    for (i = 0; i < pDeviceContext->u8Num1553Channels; i++)
    {
        ...
        {
            DDC_KERNEL_FREE(pDeviceContext->pChannel1553[i]->pu32IrqStatusQ);
            pDeviceContext->pChannel1553[i]->pu32IrqStatusQ = NULL;
        }

        if (pDeviceContext->pChannel1553[i]->sReplay.pu32IrqStatusQ)
        {
            DDC_KERNEL_FREE(pDeviceContext->pChannel1553[i]->sReplay.pu32IrqStatusQ);
        ...
        {
            DDC_KERNEL_FREE(pDeviceContext->pChannel1553[i]);
...
)
{
    ...
    u32TTResolution = (u32TTResolution & ~GENERAL_CTRL_TT_RESOLUTION_MASK) | pIoctlParams->Param1;
...
)
{
    ...
    u32TTRolloverPoint = (u32TTRolloverPoint & ~GENERAL_CTRL_TT_ROLLOVER_POINT_MASK) | pIoctlParams->Param1;
...
{
    U32BIT u32Channel = 0;
    U32BIT u32Ctrl = pIoctlParams->Param1;
...
)
{
    ...
    pTempBuffer = DDC_KERNEL_MALLOC(u32NumDwds * sizeof(U32BIT));
...
)
{
    ...
    u16SingleFpgaCh -= (pDeviceContext->sBoardInstanceIndex[u16Ch] << 1);
...
)
{
    ...
    u32Coupling = pIoctlParams->Param1;
    u32Termination = pIoctlParams->Param2;
...
)
{
    ...
    u32Amplitude = pIoctlParams->Param1;
