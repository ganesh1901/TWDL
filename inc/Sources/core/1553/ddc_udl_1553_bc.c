...
/* After turning on BC DMA, regression test observed hang in the overnight tests.
   In the earlier development phase, this also caused datastr and dataarray to receive
   3 extra messages */
#define BC_DMA                  0 /* 1- use DMA for BC, 0 - use DDC_BLK_MEM_READ */

/* Replay DMA */
#define REPLAY_DMA              1 /* 1- use DMA for replay, 0 - use PCI block write */
...
)
{
    ...
    if (pBC->u32ModeFlag & BC_MODE_FLAG_OFF_PENDING)
    {
        ...
        DDC_WAKE_UP_INTERRUPTIBLE(&pBC->waitqueueMsgEvent);
        return DDC_UDL_ERROR__BC_STATUS_MODE;
    }
...
)
{
    ...
    for (i = 0; i < u32NumMsgs; i++)
    {
        ...
        *(U16BIT*)(&(pCtrlWords->u8HostId)) = BYTE_ORDER_S(*(U16BIT*)(&(pCtrlWords->u8HostId)));
...
)
{
    ...
    {
        DDC_KERNEL_VIRTUAL_FREE(pBC->sHBuf.hBuf);
    ...
    /* allocate memory */
    pBC->sHBuf.hBuf = DDC_KERNEL_VIRTUAL_MALLOC(pBC->sHBuf.u32DwordsSize << 2);
...
{
    ACEX_BC_HBUF_INSTALL_INFO *psHBufInfo;
    ACEX_BC_BUF_SETTINGS sBuf;
    S16BIT status;

    psHBufInfo = (ACEX_BC_HBUF_INSTALL_INFO *) pIoctlParams;
    ...
    if (status)
    {
        bcHbufDisable(pDeviceContext, pIoctlParams);
    }
    else
    {
        /* enable Metric always  */
        _bcEnableHBufMetric(pDeviceContext, pIoctlParams);
    }
...
{
    ACEX_BC_BUF_SETTINGS *psBuf = (ACEX_BC_BUF_SETTINGS *) pIoctlParams;
    U16BIT u16Ch = psBuf->sConfigID.u16Channel;
...
)
{
    ...
    {
        DDC_KERNEL_FREE(pBC->sMsgBuf.hBuf);
        DDC_KERNEL_FREE(pBC->sDataBuf.hBuf);
    ...
    /* allocate memory */
    pBC->sMsgBuf.hBuf = DDC_KERNEL_MALLOC(pBC->sMsgBuf.u32Dwords << 2);
    ...
    /* allocate memory */
    pBC->sDataBuf.hBuf = DDC_KERNEL_MALLOC(pBC->sDataBuf.u32Dwords << 2);
...
)
{
    ...
    pBC->sTempBuf.pu8hBuf = DDC_KERNEL_MALLOC(pBC->sTempBuf.u32DWords << 2);
...
)
{
    ...
    {
        DDC_KERNEL_FREE(pBC->sTempBuf.pu8hBuf);
...
)
{
    ...
    U32BIT u32ReturnMsgId = pIoctlParams->Param1;
    U32BIT u32Type = pIoctlParams->Param2 & ACEX_GETMSG_TYPE;
    U32BIT u32Blocking = pIoctlParams->Param2 & ACEX_GETMSG_BLOCKING;
    U32BIT u32CheckingImp = pIoctlParams->Param2 & ACEX_GETMSG_CHECK_IMP;
    U32BIT u32WordCount = pIoctlParams->Param3;
...
)
{
    ...
    struct _ACEX_1553_BC_TYPE *pBC = &(pDeviceContext->pChannel1553[u16Ch]->sBC);
    S16BIT status = DDC_UDL_ERROR__SUCCESS;
    U32BIT u32BufType = pIoctlParams->Param1;
    U32BIT u32Reset = pIoctlParams->Param3;
    ...
    else
    {
        ...
        status = _bcGpqUpdate(pDeviceContext, pIoctlParams);
...
)
{
    ...
    struct _ACEX_1553_BC_TYPE *pBC = &(pDeviceContext->pChannel1553[u16Ch]->sBC);
    S16BIT status = DDC_UDL_ERROR__SUCCESS;
    U32BIT u32MsgIdx = pIoctlParams->Param1;
    U32BIT u32DfltMsgDwords = pIoctlParams->Param2;
    ...
    pDfltMsgHeader[ACE_BC_MSG_DATA_PTR_OFFSET] = WORD_ORDER_L(pDfltMsgHeader[ACE_BC_MSG_DATA_PTR_OFFSET]);
...
)
{
    ...
    struct _ACEX_1553_BC_TYPE *pBC = &(pDeviceContext->pChannel1553[u16Ch]->sBC);
    S16BIT status = DDC_UDL_ERROR__SUCCESS;
    U32BIT u32DataIdx = pIoctlParams->Param1;
    U32BIT u32DfltDataWords = pIoctlParams->Param2;
...
)
{
    ...
    U32BIT u32Blocking = pIoctlParams->Param2 & ACEX_GETMSG_BLOCKING;
    U32BIT u32CheckingImp = pIoctlParams->Param2 & ACEX_GETMSG_CHECK_IMP;
    U32BIT u32Purge = pIoctlParams->Param3;
...
)
{
    ...
    struct _ACEX_1553_BC_TYPE *pBC = &(pDeviceContext->pChannel1553[u16Ch]->sBC);
    S16BIT status = DDC_UDL_ERROR__SUCCESS;
    U32BIT u32DataIdx = pIoctlParams->Param1 % ACEX_BC_MAX_INDEX_MSG;
    U32BIT u32WordCount = pIoctlParams->Param2;
    U32BIT u32WordOffset = pIoctlParams->Param3;
...
)
{
    ...
    U32BIT u32Address = pIoctlParams->Param1;             /* dword address*/
    U32BIT u32Length = pIoctlParams->Param2;              /* number of 16 bit words .....architecture independent */
...
{
    U32BIT u32Address32 = pIoctlParams->Param1;
    U32BIT u32Length = pIoctlParams->Param2;
    U32BIT u32Offset = pIoctlParams->Param3;
...
)
{
    ...
    status = bcGpqGetCount(pDeviceContext, pIoctlParams, &u32BytesReturned, &u16Value);
...
)
{
    ...
    *pBytesReturned = sizeof(ACEX_BC_ASYNC_STATUS);
    ...
    u32MsgAddrAndDbpo = (pIoctlParams->Param2 & (BC_LPQ_MSG_ADDR_MASK | BC_LPQ_MSG_DBPO_MASK));
    u32MsgTime = (pIoctlParams->Param3 & BC_LPQ_MSG_TIME_MASK) << BC_LPQ_MSG_TIME_SHIFT;
...
)
{
    ...
    U16BIT u16Ch = (U16BIT)(pIoctlParams->Channel & 0xFFFF);
    U32BIT u32QueueType = pIoctlParams->Param1;
    struct _ACEX_1553_BC_TYPE *pBC = &(pDeviceContext->pChannel1553[u16Ch]->sBC);
    S16BIT status = DDC_UDL_ERROR__SUCCESS;
...
)
{
    ...
    *pBytesReturned = sizeof(ACEX_BC_ASYNC_STATUS);
    ...
    u32MsgAddrAndDbpo = (pIoctlParams->Param2 & (BC_HPQ_MSG_ADDR_MASK | BC_HPQ_MSG_DBPO_MASK));
    bMsgIrq = (BOOLEAN)((pIoctlParams->Param4 >> 31) & 0x1);
    u8HostID = (U8BIT)((pIoctlParams->Param4 >> 16) & 0x00FF);            /* 8-bit HostID */
    u16DataArrayID = (U16BIT)((pIoctlParams->Param4 & 0xFFFF) % NUM_DATARRAY_CHANNELS);
...
)
{
    ...
    u32QueueType = pIoctlParams->Param1;
...
)
{
    ...
    pDataArray->u16MsgToPostEmptyMark = (u16MsgNum * (100 - BC_DATARRAY_MSG_FULL_MARK)) / 100;
...
)
{
	...
	pDeviceContext->sDMA.pu8ReplayDMATarget = DDC_DMA_ALLOC(
		pDeviceContext,
...
)
{
    ...
    U32BIT u32MemOffset = pIoctlParams->Param2;
    U32BIT u32DataLength = pIoctlParams->Param3;
...
)
{
    ...
    U16BIT u16MemPage = (U16BIT)pIoctlParams->Param1;
    U32BIT u32MemOffset = pIoctlParams->Param2;
    U32BIT u32DataLength = pIoctlParams->Param3 >> 2; /* dwords */
...
/*-------------------------------------------------------------------------------*/
    ...
    u32RegVal = pIoctlParams->Param1; /* irq mask value */
...
)
{
    ...
    u32RespTimeout |= (pIoctlParams->Param2 << BC_CONFIG_RESP_TIMEOUT_SHIFT);
...
)
{
    ...
    for (u8DataArrayId = 0; u8DataArrayId < NUM_DATARRAY_CHANNELS; u8DataArrayId++)
    {
        ...
        pDataArray->u16MsgErrLocation = 0;
        pDataArray->u16MsgErrBsw = 0;
        pDataArray->u16MsgToPostEmptyMark = (pDataArray->u16MsgToPostMax * (100 - BC_DATARRAY_MSG_FULL_MARK)) / 100;
    }
}
...
)
{
    ...
    pBC->wMsgEventCond = 2;
    DDC_INIT_WAITQUEUE(&pBC->waitqueueMsgEvent);

    /* initialize replay DMA event */
    pBC->sReplayBuf.wDmaCmpltEventCond = 0;
    DDC_INIT_WAITQUEUE(&pBC->sReplayBuf.waitqueueDmaCmpltEvent);

    /* set to initial values */
    pBC->state = ACEX_BC_RESET;
    bcInitValues(pBC);
