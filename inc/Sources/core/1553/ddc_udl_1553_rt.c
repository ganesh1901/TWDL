...
)
{
    ...
    DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER, "ENTER->IntStatus:0x%08x ****\n", u32IntStatus);
            ...
            | MRT_INT_ENABLE_MASK_ADDR_PARITY | MRT_INT_ENABLE_MASK_ILLEGAL_CMD | MRT_INT_ENABLE_MASK_FORMAT_ERROR))
    {
        if (u32IntStatus & MRT_INT_ENABLE_MASK_CMD_STK_OVERFLOW)
        {
            ...
            DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER,
                "CMD STK OVERFLOW (num:%d)\n"
                "CumLost:%d  ThisTime:%d\n",
                pRT->stats.u32NumCmdStackOverflows++,
        ...
        {
            DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER, "ADDR_PARITY\n");
        ...
        {
            DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER, "ILLEGAL_CMD\n");
        ...
        {
            DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER, "FORMAT_ERROR\n");
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER, "ISQ_ROLLOVER\n");
        pRT->stats.u32NumIsqRollovers++;
    }

    if (u32IntStatus & MRT_INT_ENABLE_MASK_CMDSTK_50_ROLLOVER)
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER, "CMDSTK_50_ROLLOVER\n");
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER, "CBUF_50_ROLLOVER\n");
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER, "XMTR_TIMEOUT\n");
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER, "CMDSTK_100_ROLLOVER\n");
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER, "CBUF_100_ROLLOVER\n");
        ...
        {
            DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER,
                "CBUF_100_ROLLOVER - DATA ARRAY CHECK ENABLED\n");
            ...
            for (i = 0; i < (ACEX_MRT_MAX_DATA_ARRAYS * 32); i++)
            {
                ...
                {
                    DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER,
                        "Data Array %d CONTINUOUS DETECTED\n", i);
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER, "SA_CTRLWD_EOM\n");
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER, "MODE_CODE\n");
        pRT->stats.u32NumMCs++;
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER, "EOM\n");
        pRT->stats.u32NumEoms++;
        pCh->u32IrqEv |= ACE_IMR1_EOM;
    }

    DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_INTERRUPT_HANDLER, " **** mrtInterruptHandler closed ****\n");
}
...
)
{
    ...
    DDC_ISR_LOCK_INIT(pCh->sRT.semEventCond);

    pCh->sRT.u16EventCond = 0;
    DDC_INIT_WAITQUEUE(&pCh->sRT.waitqueueEvent);
...
)
{
    ...
    /* we don't have to read from hardware, because USB is always INTERNAL */
    /* <UDL3> */
...
)
{
    ...
    *pBytesReturned = 0;
    ...
    if (pRtAccess->u32Data == ACEX_MRT_HBUF_GET)
    {
        ...
        {
            u32NumOfMsg = OutputBufferLength / (ACE_MSGSIZE_RT * 2);
    ...
    else
    {
        ...
        switch (pRtAccess->u32Data)
        {
            case ACEX_MRT_HBUF_ENABLE:
            {
                ...
                pCh->sRT.sHbuf.pu8hbufMemory = DDC_KERNEL_MALLOC(u32CmdBufByteSize);
            ...
            case ACEX_MRT_HBUF_DISABLE:
            {
                ...
                DDC_KERNEL_FREE(pCh->sRT.sHbuf.pu8hbufMemory);
...
)
{
    ...
    U32BIT u32CmdBlk[RT_CMD_SIZE];
    U16BIT u16RtAddr;
    U16BIT u16SubAddr;
    U16BIT u16WordCount;

    U32BIT u32CmdStkPointerBA;
    U32BIT u32CmdStkDataAddr;
    
    U32BIT u32CmdStkPointer = 0;
    */
    struct _ACEX_1553_RT_TYPE *pRT = NULL;

    S16BIT status = DDC_UDL_ERROR__SUCCESS;

    DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_IMP, "ENTER->\n");

    pRT = &(pDeviceContext->pChannel1553[u16Channel]->sRT);

    /* return bytes count */
    *pBytesReturned = 0;
    pRdData[0] = *pBytesReturned;

    /* get all requested cmds */
    if (u32NumCmd == 0)
    {
        return status;
    }

    /* TT1639 fix: do not read message from RT cmd/data stack directly. 
       There is no enough information about data block size and RT/SA mapping. 
     */
#if 0
    if (u32NumCmd == 1)
    {
        pPackedMsg = (U16BIT *)pRdData;

        u32Register = (*(pRT->pu32RegBA)) + REG_MRT_CMD_STK_PTR;
        DDC_REG_READ(pDeviceContext, u32Register, &u32CmdStkPointer);
        u32CmdStkPointerBA = pRT->sRtxCfgReg.u32CmdStkPtr & pDeviceContext->pChannel1553[u16Channel]->u32ChannelMemDwdMask;
        if (u32CmdStkPointer < u32CmdStkPointerBA)
        {
            DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_IMP, 
                "Ch%d: u32CmdStkPointer 0x%08x TOO SMALL, BA 0x%08x, SW index 0x%08x\n", 
                u16Channel, u32CmdStkPointer, u32CmdStkPointerBA, pRT->sRtxCfgReg.u32CmdStkPtrSwIndex);

            return status;
        }

        if (u32CmdStkPointer > (u32CmdStkPointerBA + pRT->sCmdStk.u32RtCmdStkDwdSz - RT_CMD_SIZE))
        {
            DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_IMP, 
                "Ch%d: u32CmdStkPointer 0x%08x TOO BIG, BA 0x%08x, SW index 0x%08x\n", 
                u16Channel, u32CmdStkPointer, u32CmdStkPointerBA, pRT->sRtxCfgReg.u32CmdStkPtrSwIndex);

            return status;
        }

        if (u32CmdStkPointer == (u32CmdStkPointerBA + pRT->sRtxCfgReg.u32CmdStkPtrSwIndex))
        {
            DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_IMP, 
                "Ch%d: u32CmdStkPointer 0x%08x NO CMD, BA 0x%08x, SW index 0x%08x\n", 
                u16Channel, u32CmdStkPointer, u32CmdStkPointerBA, pRT->sRtxCfgReg.u32CmdStkPtrSwIndex);
        ...
        DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_IMP, 
            "Ch%d: u32CmdStkPointer 0x%08x, BA 0x%08x, Dwd Size 0x%08x, SW index 0x%08x\n", 
            u16Channel, u32CmdStkPointer, pRT->sRtxCfgReg.u32CmdStkPtr, pRT->sCmdStk.u32RtCmdStkDwdSz, pRT->sRtxCfgReg.u32CmdStkPtrSwIndex);

        /* there is a msg, skip the header and point to the cmd area */
        pPackedMsg += 2;
        *pBytesReturned += (2 << 1);
        pRdData[0] = *pBytesReturned;

        /* read cmd stk */
        DDC_BLK_MEM_READ(pDeviceContext, pRT->sRtxCfgReg.u32CmdStkPtr + pRT->sRtxCfgReg.u32CmdStkPtrSwIndex, (U32BIT *)u32CmdBlk, RT_CMD_SIZE, ACEX_32_BIT_ACCESS_16_BIT_HW_MEM);

        /* copy low 16-bits to user */
        pPackedMsg[0] = (U16BIT)(u32CmdBlk[0] & 0xFFFF);
        pPackedMsg[1] = (U16BIT)(u32CmdBlk[1] & 0xFFFF);
        pPackedMsg[2] = (U16BIT)(u32CmdBlk[2] & 0xFFFF);
        pPackedMsg[3] = (U16BIT)(u32CmdBlk[3] & 0xFFFF);

        *pBytesReturned += (RT_CMD_SIZE << 1);
        pRdData[0] = *pBytesReturned;

        DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_IMP, 
            "Ch%d: cmd stk BSW 0x%04x, TT 0x%04x, DP 0x%08x, CMD 0x%04x\n", 
            u16Channel, u32CmdBlk[0], u32CmdBlk[1], u32CmdBlk[2], u32CmdBlk[3]);

        /* point to the next cmd */
        pRT->sRtxCfgReg.u32CmdStkPtrSwIndex += RT_CMD_SIZE;
        if (pRT->sRtxCfgReg.u32CmdStkPtrSwIndex >= pRT->sCmdStk.u32RtCmdStkDwdSz)
        {
            pRT->sRtxCfgReg.u32CmdStkPtrSwIndex = 0;
            DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_IMP, 
                "Ch%d: u32CmdStkPointer rollover, BA 0x%08x, Dwd Size 0x%08x, SW index 0x%08x\n", 
                u16Channel, pRT->sRtxCfgReg.u32CmdStkPtr, pRT->sCmdStk.u32RtCmdStkDwdSz, pRT->sRtxCfgReg.u32CmdStkPtrSwIndex);
        }

        /* no data to read if it is mode code command */
        u16SubAddr = (pPackedMsg[RT_CMD_OFFSET] >> 5) & 0x1F;
        u16RtAddr = (pPackedMsg[RT_CMD_OFFSET] >> 11) & 0x1F;

        if (u16SubAddr == 0 || ((u16SubAddr == 0x1F) && !pRT->b1553a[u16RtAddr]))
        {
            /* return now for mode code commands */
            return status;
        }

        /* calculate data length */
        u16WordCount = (pPackedMsg[RT_CMD_OFFSET]) & 0x1F;
        if (u16WordCount == 0 || u16WordCount > 32)
        {
            u16WordCount = 32;
        }

        /* return if user buffer can not hold all data */
        if (u16WordCount > ((u32OutputBufferLen - *pBytesReturned) >> 1))
        {
            DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_IMP, 
            "Ch%d: buffer size with %d words are smaller than data word count %d\n", 
            u16Channel, u32OutputBufferLen / 2, u16WordCount);
            
            return status;
        }

        /* calculate word address for cmd data */
        u32CmdStkDataAddr = (*(pDeviceContext->pChannel1553[u16Channel]->pu32MemBA))*2 +
            (u32CmdBlk[RT_DP_OFFSET] & pDeviceContext->pChannel1553[u16Channel]->u32ChannelMemDwdMask);

        /* read cmd data in word */
        DDC_16BIT_BLK_MEM_READ(pDeviceContext,
            u32CmdStkDataAddr,                      /* word address */
            pPackedMsg + RT_CMD_SIZE,               /* point to data in user buffer */
            u16WordCount);                          /* word count */
            
        DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_IMP, "Ch%d: dataPointer 0x%04x %d words, data 0x%04x, 0x%04x, 0x%04x, 0x%04x\n", 
            u16Channel, pPackedMsg[2], u16WordCount, pPackedMsg[4], pPackedMsg[5], pPackedMsg[6], pPackedMsg[7]);

        /* return bytes count */
        *pBytesReturned += (u16WordCount << 1);
        pRdData[0] = *pBytesReturned;
        return status;
    }
#endif
    ...
    DDC_ISR_LOCK_TAKE(pRT->semEventCond, pRT->semEventCondFlag);
    if (pRT->u16EventCond)
    {
        pRT->u16EventCond--;
    }
    DDC_ISR_LOCK_GIVE(pRT->semEventCond, pRT->semEventCondFlag);
    
    DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_IMP, "Ch %d requesting...%d cmds\n", u16Channel, u32NumCmd);
    ...
    *pBytesReturned = pRT->u32RdByte;
    ...
    DDC_DBG_PRINT(DDC_DBG_MODULE_RT, DDC_DBG_RT_IMP, "Ch %d Bytes Returned: %d\n", u16Channel, (unsigned int)(*pBytesReturned));
...
#if DDC_PPC
        MCDAddrOffset = wModeCode & ~(ACE_RT_MCDATA_BCST_CODE_BITMASK);
        MCDAddrOffset = (MCDAddrOffset - (MCDAddrOffset & 0x1)) + !(MCDAddrOffset & 0x1);
#else
        MCDAddrOffset = wModeCode & ~(ACE_RT_MCDATA_BCST_CODE_BITMASK);
#endif
    }
