...
)
{
#if DDC_TRANSFER_VIA_DMA_717
    {
        ...
        for (i = 0; i < pDeviceContext->u8NumProg717; i++)
        {
            ...
            {
                DDC_KERNEL_FREE(pDeviceContext->sChannelArinc717[i].pu8MemoryBuf);
...
)
{
    ...
    U32BIT u32Option = pIoctlParams->Param2;

    if (pDeviceContext->eArinc717ProgState[u8Channel] == ARINC_717_RESET)
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC717, DDC_DBG_ARINC717_TX_LOAD, "ARINC717LoadTxQueueData - Ch %ld, Invalid state for operation.\n",
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC717, DDC_DBG_ARINC717_TX_LOAD, "ARINC717LoadTxQueueData - Ch %d Tx load size does not match for speed setting. Is %ld bytes, Should be %d\n",
            (u8Channel + 1), (pIoctlParams->Param1 / sizeof(U32BIT)), u32DWords);
    ...
    /*
     *      ARINC 717 Double Buffer Mode Memory Configurarion
     *
...
#if DDC_TRANSFER_VIA_DMA_717
    {
        ...
        nResult = DDC_WAIT_INTERRUPTABLE_TIMEOUT_MS(
            pDeviceContext->waitqueueARINC717TxFrame[u8Channel],
...
#else
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC717, DDC_DBG_ARINC717_TX_LOAD, "ARINC717LoadTxQueueData - Ch %ld loading %d DWs at start address 0x%X\n",
...
)
{
    ...
    *pBytesReturned = 0;
    if (pDeviceContext->eArinc717ProgState[u8Channel] == ARINC_717_RESET)
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC717, DDC_DBG_ARINC717_RX_READ, "ARINC717ReadData - Ch %ld, Invalid state for operation.\n",
            pIoctlParams->Channel);
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC717, DDC_DBG_ARINC717_RX_READ, "ARINC717ReadData - Ch %d Tx load size does not match for speed setting. Is %ld bytes, Should be %d\n",
            (u8Channel + 1), (pIoctlParams->Param1 / sizeof(U32BIT)), u32DWords);
    ...
    DDC_DBG_PRINT(DDC_DBG_MODULE_ARINC717, DDC_DBG_ARINC717_RX_READ, "ARINC717ReadData - Ch%ld reads %d DWs at start address 0x%X\n",
        pIoctlParams->Channel, u32DWords, u32AddrBaseAddress);
...
#if DDC_TRANSFER_VIA_DMA_717
    {
        ...
        nResult = DDC_WAIT_INTERRUPTABLE_TIMEOUT_MS(
            pDeviceContext->waitqueueARINC717RxFrame[u8Channel],
