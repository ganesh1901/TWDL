...
{
    U32BIT dwAddress = (pIoctlParams->Param1) >> 1; /*DDC_1553_ADDR_SHIFT*/ /* from 8bit addr to 16bit addr */
    U32BIT dwData = 0;
    U8BIT bStatus;

    dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_INFO, "Erase dwAddress = 0x%x!\n", (int)dwAddress);
    ...
    dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_INFO, "bStatus = 0x%u!\n", bStatus);
...
{
    U32BIT dwAddress = (pIoctlParams->Param1) >> 1;        /* from 8bit addr to 16bit addr */
    U32BIT dwByteCount = pIoctlParams->Param2;
    ...
    for (i = 0; i < dwByteCount; i += 2)
    {
        ...
        /* Write the C/D-In word to the FLASH C/D-In register */
        DDC_REG_WRITE(pDeviceContext, (*(pDeviceContext->pBoardInfo[ACEX_BD_INSTANCE_0]->pu32RegBA) + REG_BD_FLASH_CONTROL), &dwData);
        dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_INFO, "Write Reg (Flash Ctrl) dwData =0x%x!\n", (int)dwData);
        ...
        dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_INFO, "Read Reg (Flash Status) dwData =0x%x!\n", (int)dwData);

        if (dwTimeoutLoop == FLASH_OP_TIMEOUT)
        {
            /* fail IOCTL */
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_ERROR, "Time Out!\n");
            return dwData;
        }
        ...
        /* Clear C/D-In perform op bit */
        DDC_REG_READ(pDeviceContext, (*(pDeviceContext->pBoardInfo[ACEX_BD_INSTANCE_0]->pu32RegBA) + REG_BD_FLASH_CONTROL), &dwData);
        dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_INFO, "Read Reg (Flash Ctrl) dwData =0x%x!\n", (int)dwData);
        dwData &= FLASH_OP_TERM;
        DDC_REG_WRITE(pDeviceContext, (*(pDeviceContext->pBoardInfo[ACEX_BD_INSTANCE_0]->pu32RegBA) + REG_BD_FLASH_CONTROL), &dwData);
        dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_INFO, "Write Reg (Flash Ctrl) dwData =0x%x!\n\n", (int)dwData);
        dwTimeoutLoop = 0;
        ...
        if (dwTimeoutLoop == FLASH_OP_TIMEOUT)
        {
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_ERROR, "dwData =0x%x!\n", (int)dwData);
            return bStatus;
        }
        ...
        if (bStatus)
        {
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_ERROR, "bStatus =0x%x!\n", bStatus);
            return bStatus;
        }
...
)
{
    ...
    dwAddress = (pIoctlParams->Param1) >> 1;   /*DDC_1553_ADDR_SHIFT*/ /* from 8bit addr to 16bit addr */
    pData = (U8BIT *)IOBuffer;
    dwByteCount = pIoctlParams->Param2;

    dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_INFO, "dwAddress : 0x%x  dwByteCount : 0x%x \n", (int)dwAddress, (int)dwByteCount);
    ...
    for (i = 0; i < dwByteCount; i += 2)
    {
        ...
        if (dwTimeoutLoop == FLASH_OP_TIMEOUT)
        {
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_ERROR, "operation 1  timed out!\n");
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_ERROR, "dwData = 0x%x\n", (int)dwData);
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_ERROR, "last op = 0x%x\n", (int)dwDataOp);
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_ERROR, "last op addr = 0x%x\n", (int)OpAddr);
        ...
        {
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_ERROR, "operation 2 fails!\n");
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_ERROR, "bStatus =0x%x\n", bStatus);
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_ERROR, "dwData =0x%x\n", (int)dwData);
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_ERROR, "last op = 0x%x\n", (int)dwDataOp);
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_ERROR, "last op addr = 0x%x\n", (int)OpAddr);
        ...
        if ((i / 2) < 10)
        {
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_INFO, "dwData = 0x%x\n", (int)dwData);
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_INFO, "last op = 0x%x\n", (int)dwDataOp);
            dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_INFO, "last op addr = 0x%x\n", (int)OpAddr);
        }
    }
...
)
{
    ...
    dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_FUNC, "ENTER->\n");

    retval = DDC_REG_READ(pDeviceContext, (*(pDeviceContext->pBoardInfo[ACEX_BD_INSTANCE_0]->pu32RegBA) + REG_BD_STATUS), &Value);

    if (retval)
    {
        dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_ERROR, "Read Flash Mem Write Protected Error!\n");
        return retval;
    }

    dbg_print(DBG_MODULE_FLASH, DBG_PRIORITY_INFO, "MemWriteProtect 0x%08x\n", Value);
