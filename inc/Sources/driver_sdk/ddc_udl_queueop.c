...
)
{
    ...
    pHostQ = (Q_INFO_TYPE *)DDC_KERNEL_MALLOC((u32SizeOfEntry * u32NumQueEntries) + sizeof(Q_INFO_TYPE));
...
)
{
    ...
    dbg_print(DBG_MODULE_QUEUE, DBG_PRIORITY_INFO, "Q__INFO_TYPE_Destroy : psHostQ->u32MaxEntriesInQueue =%d\n", psHostQ->u32MaxEntriesInQueue);

    DDC_KERNEL_FREE(psHostQ);
...
)
{
    ...
    dbg_print(DBG_MODULE_QUEUE, DBG_PRIORITY_INFO, "Q__INFO_TYPE_Get_Tail : psHostQ->u32QueueSize =0x%x\n", psHostQ->u32QueueSize);
    dbg_print(DBG_MODULE_QUEUE, DBG_PRIORITY_INFO, "Q__INFO_TYPE_Get_Tail : psHostQ->u32EntriesInQueue =0x%x\n", psHostQ->u32EntriesInQueue);
...
)
{
    ...
    dbg_print(DBG_MODULE_QUEUE, DBG_PRIORITY_INFO, "Q__INFO_TYPE_Add : psHostQ->u32QueueSize =0x%x\n", psHostQ->u32QueueSize);
    dbg_print(DBG_MODULE_QUEUE, DBG_PRIORITY_INFO, "Q__INFO_TYPE_Add : psHostQ->u32EntriesInQueue =0x%x\n", psHostQ->u32EntriesInQueue);

    if (psHostQ->u32EntriesInQueue >= psHostQ->u32QueueSize)
    {
        dbg_print(DBG_MODULE_QUEUE, DBG_PRIORITY_ERROR, "Q__INFO_TYPE_Add : error condition \n");
        return NULL;
    }
...
)
{
    ...
    if ((psHostQ->u32NextRead == psHostQ->u32LastWritten) && (psHostQ->u32EntriesInQueue == 0)) /* Q is empty */
    {
        dbg_print(DBG_MODULE_QUEUE, DBG_PRIORITY_ERROR, "EMPTY Q\n");
        return NULL;
    }
