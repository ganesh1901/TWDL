...
)
{
    ...
    pDeviceContext->sHwVersionInfo.dwFamilyNumber = DDC_ACEX;
...
)
{
    ...
    dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "Board Model Number updated to %s\n",
        pDeviceContext->pUmInfo->s8BrdModelNum);
    dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "QPRM Bus %d Dev %d attached to its CPLD device!\n",
...
)
{
    ...
    dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "ioDevBindingQprmCallBackFromCPLD: ENTER ---\n");

    /* exit if CPLD device context is NULL */
    if (!pDevConCPLD)
    {
        dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_ERROR, "CPLD device unknown\n");
        return;
    }
    ...
    {
        dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_ERROR, "Exit because it is not notified by CPLD\n");
        return;
    }
    ...
    {
        pDeviceContext = ddcUdlGetDeviceContext(i);

        if (!pDeviceContext)
        {
            dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "Skipped because it is not a valid device desciptor\n");
            continue;
        }
        ...
        if (pDeviceContext->u16DriverType != ACEX_QPRM_DRIVER)
        {
            dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "Skipped because it is not a QPRM device\n");
            continue;
        }
        ...
        {
            dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "Skipped because the device load is not completed\n");
            continue;
        }
        
        dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "QPRM %d pDev 0x%p on Bus %d notified by dev %d pDev 0x%p\n",
        ...
        {
            dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "Skipped because QPRM is on Bus %d but CPLD is on Bus %d\n",
        ...
        {
            dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "Skipped because QPRM is on Dev %d but CPLD is on Dev %d\n",
                pDeviceContext->ddcOsDevInfo.sBusInfo.u16PciDevNum, pDevConCPLD->ddcOsDevInfo.sBusInfo.u16PciDevNum);
            continue;
        }

        /* update QPRM device */
        dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "ioDevBindingQprmCallBackFromCPLD: CPLD %d:%d binding with QPRM %d:%d\n",
    ...
    dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "ioDevBindingQprmCallBackFromCPLD: EXIT ---\n");
}
...
)
{
    ...
    dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "ioDevBindingCpldCallBackFromQPRM: ENTER ===\n");

    /* exit if device context is NULL */
    if (!pDeviceContext)
    {
        dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "unknown device\n");
        return;
    }
    ...
    if (pDeviceContext->u16DriverType != ACEX_QPRM_DRIVER)
    {
        dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "Exit because it is not notified by QPRM\n");
        return;
    }
    ...
    for (i = 0; i < u8CardCount; i++)
    {
        pDevConCPLD = ddcUdlGetDeviceContext(i);
        
        if (!pDevConCPLD)
        {
            dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "Skipped because it is not a valid device descriptor\n");
            continue;
        }
        ...
        if (pDevConCPLD->u16DriverType != ACEX_IO_DRIVER)
        {
            dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_ERROR, "Skipped because it is not a CPLD module\n");
            continue;
        }

        dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "CPLD %d pDev 0x%p on Bus %d notified by dev %d pDev 0x%p\n",
        ...
        dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "ioDevBindingCpldCallBackFromQPRM: QPRM %d:%d binding with CPLD %d:%d\n",
            pDeviceContext->ddcOsDevInfo.sBusInfo.u16PciBusNum, pDeviceContext->ddcOsDevInfo.sBusInfo.u16PciDevNum,
            pDevConCPLD->ddcOsDevInfo.sBusInfo.u16PciBusNum, pDevConCPLD->ddcOsDevInfo.sBusInfo.u16PciDevNum);
    ...
    dbg_print(DBG_MODULE_IODEV, DBG_PRIORITY_INFO, "ioDevBindingCpldCallBackFromQPRM: EXIT ===\n");
}
