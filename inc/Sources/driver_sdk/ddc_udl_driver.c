 ...
 * Out  none
 *
 * Returns: const poitner to supported device list
 ******************************************************************************/
...
)
{
    ...
    pDevices[u8DeviceCount] = DDC_KERNEL_MALLOC(sizeof(struct _DDC_UDL_DEVICE_CONTEXT));
...
)
{
    ...
    /* free MTI data packet memory - safe to call even if device does not support 1553 */
 ...
 * Returns: error condition
 ******************************************************************************/
static S16BIT _ddcUdlDeviceContexInit
(
    struct _DDC_UDL_DEVICE_CONTEXT *pDeviceContext
)
{
    ...
    /*  initialize IO only device if find one*/
    ...
    /* Reset all componets of the device */
    ...
    for (u8ChannelNumber = 0; u8ChannelNumber < pDeviceContext->u8Num1553Channels; u8ChannelNumber++) /* 1553 channel for loop */
    {
        ...
        pDeviceContext->pChannel1553[u8ChannelNumber]->pu32IrqStatusQ = DDC_KERNEL_MALLOC((IRQ_STATUS_QUEUE_SIZE *
        ...
        pDeviceContext->pChannel1553[u8ChannelNumber]->sReplay.pu32IrqStatusQ = DDC_KERNEL_MALLOC((REPLAY_IRQ_STATUS_QUEUE_SIZE * sizeof(U32BIT)));
        ...
        ||(pDeviceContext->u32DeviceMask & UM_DEVICE_MASK_MIO_CAST_UART_SDLC_HDLC))
    {
        ...
        DDC_INIT_WAITQUEUE(&pDeviceContext->waitqueueArincBlockOnIrqEvent);
        ...
        {
            DDC_INIT_WAITQUEUE(&pDeviceContext->pRxChnl429[i]->waitqueueARINC429RxFifo);
            DDC_INIT_WAITQUEUE(&pDeviceContext->pTxChnl429[i]->waitqueueARINC429SetTxFrameControl);
        }
    }
    ...
    if (pDeviceContext->sHwVersionInfo.dwCapabilities & HWVER_CAPABILITY_PROG_ARINC_717)
    {
        ...
        {
            DDC_INIT_WAITQUEUE(&pDeviceContext->waitqueueARINC717TxFrame[i]);
            DDC_INIT_WAITQUEUE(&pDeviceContext->waitqueueARINC717RxFrame[i]);
        }
    ...
    DDC_INIT_WAITQUEUE(&pDeviceContext->waitqueueArinc429VoltageMonitoring);

    /* Allocate device level DMA Descriptor (less than 4GB) */
    if (!(pDeviceContext->sDMA.pu32Descriptor = (U32BIT *) DDC_KERNEL_GET_FREE_PAGE()))
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_DEVICE, DDC_DBG_DEVICE_INIT, "DDC_KERNEL_GET_FREE_PAGE fail");
        return DDC_UDL_ERROR__INSUFFICIENT_MEMORY;
    }
    ...
    /* Allocate RT DMA buffer */
    pDeviceContext->pu8RtDmaTarget = DDC_DMA_ALLOC(
        pDeviceContext,
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_DEVICE, DDC_DBG_DEVICE_INIT, "DDC_DMA_ALLOC fail");
        return DDC_UDL_ERROR__INSUFFICIENT_MEMORY;
    }
        ...
        (pDeviceContext->u16DriverType == ACEX_DD429_PCI_DRIVER))
    {
        ...
        if (pDeviceContext->pRxChnl429[0]->pu32MemSize)
        {
            ...
            pDeviceContext->ARINC429RxDMATarget = DDC_DMA_ALLOC(
                pDeviceContext,
            ...
            {
                DDC_DBG_PRINT(DDC_DBG_MODULE_DEVICE, DDC_DBG_DEVICE_INIT, "DDC_DMA_ALLOC for ARINC429RxDMATarget failed");
                return DDC_UDL_ERROR__INSUFFICIENT_MEMORY;
            }
            ...
            pDeviceContext->ARINC429TxDMATarget = DDC_DMA_ALLOC(
                pDeviceContext,
            ...
            {
                DDC_DBG_PRINT(DDC_DBG_MODULE_DEVICE, DDC_DBG_DEVICE_INIT, "DDC_DMA_ALLOC for ARINC429TxDMATarget failed");
                return DDC_UDL_ERROR__INSUFFICIENT_MEMORY;
            }
        ...
        if (pDeviceContext->pBoardInfo[ACEX_BD_INSTANCE_0]->sBdVoltMonX8.pu32MemSize)
        {
            ...
            pDeviceContext->Arinc429VoltageMonDMATarget = DDC_DMA_ALLOC(
                pDeviceContext,
            ...
            {
                DDC_DBG_PRINT(DDC_DBG_MODULE_DEVICE, DDC_DBG_DEVICE_INIT, "DDC_DMA_ALLOC for VoltageMonDMATarget failed");
                return DDC_UDL_ERROR__INSUFFICIENT_MEMORY;
            }
        ...
        if (pDeviceContext->sChannelArinc717[0].pu32MemSize)
        {
            ...
            pDeviceContext->ARINC717RxDMATarget = DDC_DMA_ALLOC(
                pDeviceContext,
            ...
            {
                DDC_DBG_PRINT(DDC_DBG_MODULE_DEVICE, DDC_DBG_DEVICE_INIT, "DDC_DMA_ALLOC for ARINC717RxDMATarget failed");
                return DDC_UDL_ERROR__INSUFFICIENT_MEMORY;
            }
            ...
            for (i=0; i < pDeviceContext->u8NumProg717; i++)
            {
                ...
                pDeviceContext->sChannelArinc717[i].pu8MemoryBuf = DDC_KERNEL_MALLOC(u32DmlBytes);
                if (!pDeviceContext->sChannelArinc717[i].pu8MemoryBuf)
                {
                    DDC_DBG_PRINT(DDC_DBG_MODULE_DEVICE, DDC_DBG_DEVICE_INIT, "DDC_DMA_ALLOC for Arinc 717 ch%du8DmaMemory failed", i);
                    return DDC_UDL_ERROR__INSUFFICIENT_MEMORY;
                }
            ...
            pDeviceContext->ARINC717TxDMATarget = DDC_DMA_ALLOC(
                pDeviceContext,
            ...
            {
                DDC_DBG_PRINT(DDC_DBG_MODULE_DEVICE, DDC_DBG_DEVICE_INIT, "DDC_DMA_ALLOC for ARINC717TxDMATarget failed");
                return DDC_UDL_ERROR__INSUFFICIENT_MEMORY;
            }
            DDC_DBG_PRINT(DDC_DBG_MODULE_DEVICE, DDC_DBG_DEVICE_INIT, "ARINC717TxDMATarget=%p\n",pDeviceContext->ARINC717TxDMATarget);
        }
#endif /* DDC_TRANSFER_VIA_DMA_717 */
    ...
    for (u8ChannelNumber = 0; u8ChannelNumber < pDeviceContext->u8Num1553Channels; u8ChannelNumber++)
    {
        ...
        DDC_INIT_WAITQUEUE(&pDeviceContext->pChannel1553[u8ChannelNumber]->waitqueueIrqEvent);
        ...
        DDC_INIT_WAITQUEUE(&pDeviceContext->pChannel1553[u8ChannelNumber]->sReplay.waitqueueIrqEvent);

        /* Initialize MTI data and time packet wait queues */
        pDeviceContext->pChannel1553[u8ChannelNumber]->sMT.u8MtiDataPoolEventFlag = 0;
        DDC_INIT_WAITQUEUE(&pDeviceContext->pChannel1553[u8ChannelNumber]->sMT.eMtiDataPoolEvent);

        pDeviceContext->pChannel1553[u8ChannelNumber]->sMT.u8MtiTimePoolEventFlag = 0;
        DDC_INIT_WAITQUEUE(&pDeviceContext->pChannel1553[u8ChannelNumber]->sMT.eMtiTimePoolEvent);
    }
...
)
{
    ...
    for (u8DeviceNumber = 0; u8DeviceNumber < u8DeviceCount; u8DeviceNumber++)
    {
        ...
        pDeviceContext->ddcOsDevInfo.u8DeviceNumber = u8DeviceCount;

        if (_ddcUdlDeviceContexInit(pDeviceContext))
        {
...
)
{
    ...
    for (u8DeviceNumber = 0; u8DeviceNumber < u8NumberDevicesInstalled; u8DeviceNumber++)
    {
        ...
        if (pDeviceContext)
        {
            /* Unmap register space if it is not an I/O only device */
            if (pDeviceContext->u16DriverType != ACEX_IO_DRIVER)
            {
                /* free page descriptor */
                DDC_KERNEL_FREE_PAGE((long)pDeviceContext->sDMA.pu32Descriptor);
            ...
            if (pDeviceContext)
            {
                DDC_KERNEL_FREE(pDeviceContext);
