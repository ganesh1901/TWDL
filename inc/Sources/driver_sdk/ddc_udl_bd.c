...
)
{
    ...
    for (i = 0; i < MAX_NUM_FPGAS; i++)
    {
        ...
        {
            DDC_KERNEL_FREE(pDeviceContext->pBoardInfo[i]);
...
)
{
    ...
    for (i = 0; i < MAX_NUM_FPGAS; i++)
    {
        ...
        pDeviceContext->pBoardInfo[i] = DDC_KERNEL_MALLOC(sizeof(ACEXPCI_BOARD_TYPE));
...
)
{
        ...
        switch (pDeviceContext->pBoardInfo[ACEX_BD_INSTANCE_0]->sBdReg.u32ModelNum)
        {
            ...
            case 10:
            {
                ...
                pDeviceContext->sHwVersionInfo.szModelName[DD40X00X_MODEL_VARIANT_START] =
                    '0' + (U8BIT)(pDeviceContext->pBoardInfo[ACEX_BD_INSTANCE_0]->sBdReg.u32ModelNum / 10);
    ...
    {
        pDeviceContext->sHwVersionInfo.dwCapabilities |= HWVER_CAPABILITY2_DIO_INTERRUPT;
    }
...
)
{
    ...
    pBuffer = DDC_KERNEL_MALLOC(u32BufByteLenReq);
    ...
    DDC_16BIT_BLK_MEM_WRITE(pDev, u32Address16, pBuffer, u32NumWds);

    DDC_KERNEL_FREE(pBuffer);
    ...
    switch (pIoctlParams->Param1)
    {
        case DDC_BOARD_STATUS_CMD_TEMPERATURE:
        {
            ...
            for (i = 0; i < MAX_TEMPERATURE_DEVICE_NUMBER; i++)
            {
                ...
                pdata[2 + i] = (S16BIT)((dwData & 0x0fff) >> 4);
                ...
                if (((DEVICE_TEMPERATURE_TYPE *)pdata)->u32_Options_Flags & DEVICE_TEMPERATURE_FAHRENHEIT)
                {
                    pdata[2 + i] = (180 * pdata[2 + i] + 3200) / 100; /* Celsius to Fahrenheit conversion formula */
                }
            }

            /* Tell caller we returned all temperatures */
            ((DEVICE_TEMPERATURE_TYPE *)pdata)->u32_Options_Flags |= DEVICE_TEMPERATURE_ALL;
            break;
        }
