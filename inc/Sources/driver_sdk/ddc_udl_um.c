...
)
{
    ...
    /* set PPC */
    u32Data = 0xFFFFFFFF;
    DDC_REG_WRITE(pDeviceContext, 0x0, &u32Data);

    /* Read board model number and check whether the order is OK.
       If not, then the current firmware release doesn't support
       the 'hardware byte and word swap' feature. Implementation
       is meant to support the flash utility to flash releases
       prior to 5.0.
     */
    status = DDC_REG_READ(pDeviceContext, 0x00000000, &u32Data);
    ...
    DDC_DBG_PRINT(DDC_DBG_MODULE_UM, DDC_DBG_UM_INIT, "Endianness Mode reads %s\n", s8BrdModelNum);
...
{
    pDeviceContext->pUmInfo = DDC_KERNEL_MALLOC(sizeof(struct _UM_INFO_TYPE));
...
)
{
    ...
    {
        DDC_KERNEL_FREE(pDeviceContext->pUmInfo);
