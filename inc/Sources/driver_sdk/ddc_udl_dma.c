...
)
{
    ...
    ptr = (DESCRIPTOR_POINTER_ADDR(DDC_VIRT_TO_PHYS(pDeviceContext->sDMA.pu32Descriptor)) << 4);
...
)
{
    ...
    else
    {
        ...
        pDeviceContext->sDMA.pQueuePtr = Q__INFO_TYPE_Create(sizeof(DMA_Q_ENTRY), u32QueueSize);
...
)
{
    ...
    Q__INFO_TYPE_Destroy(pDeviceContext->sDMA.pQueuePtr);
...
)
{
    ...
    dtePhysicalAddress = (U32BIT)(DDC_VIRT_TO_PHYS(dteVirtualAddress));
    dtePhysicalAddress += (U32BIT)(sizeof(DMA_TRANSFER_ELEMENT));
    ...
    if ((pDMATransaction->StartAddr + pDMATransaction->ByteCount) > pDMATransaction->u32DMACHMaxAddr)
    {
        ...
        dtePhysicalAddress += sizeof(DMA_TRANSFER_ELEMENT);
...
#if DDC_PPC
{
    ...
    if (pDeviceContext->u16DriverType != ACEX_QPRM_DRIVER)
    {
        ...
        dtePhysicalAddressTemp = (U32BIT)(DDC_VIRT_TO_PHYS(dteVirtualAddress) + sizeof(DMA_TRANSFER_ELEMENT));

        for (i = 0; i < pDMATransaction->SgListNumEntries; i++)
        {
            /* adjust endianness of descriptor */
            dteVirtualAddressTemp->PciAddressLow = BYTE_ORDER_L(dteVirtualAddressTemp->PciAddressLow);

            /* dteVirtualAddressTemp->PciAddressHigh is always 0, so there is no need to swap */
            dteVirtualAddressTemp->TransferSize = BYTE_ORDER_L(dteVirtualAddressTemp->TransferSize);
            dteVirtualAddressTemp->LocalAddress = BYTE_ORDER_L(dteVirtualAddressTemp->LocalAddress);
            dteVirtualAddressTemp->NextDescriptorPointer = BYTE_ORDER_L(dteVirtualAddressTemp->NextDescriptorPointer);

            /*Adjust the next DMA_TRANSFER_ELEMEMT */
            dteVirtualAddressTemp++;
            dtePhysicalAddressTemp += sizeof(DMA_TRANSFER_ELEMENT);
        }
    }
}
#endif /* DDC_PPC */
    ...
    DDC_ISR_LOCK_TAKE(pDeviceContext->sDMA.semDmaQ, pDeviceContext->sDMA.semDmaQFlag);
    pDeviceContext->sDMA.u16CurrentChannel = pDMATransaction->Channel + 1;
    DDC_ISR_LOCK_GIVE(pDeviceContext->sDMA.semDmaQ, pDeviceContext->sDMA.semDmaQFlag);
...
)
{
    ...
    {
        DDC_DBG_PRINT(DDC_DBG_MODULE_DMA, DDC_DBG_DMA_MT, "MTI: Adjust Addr, back %d bytes\n", DMA_ALIGNMENT_BYTES - u32AlignAddr);

        DmaQueueEntry.copyOffset = DMA_ALIGNMENT_BYTES - u32AlignAddr;
...
#if DEBUG_MTI_CH10_DMA
{
    ...
    DDC_WAKE_UP_INTERRUPTIBLE(&pMT->eMtiDataPoolEvent);
}
...
)
{
    ...
    pRT->sHbuf.u32HbufLstWr = pRT->sHbuf.u32HbufLstWr + pDmaEntry->RequestedByteCount;
    pRT->sHbuf.u32CmdIndex += pDmaEntry->U.MrtHbufParam.u32NumCmdXfer; /* adjust our index */
...
)
{
    ...
    DDC_WAKE_UP_INTERRUPTIBLE(&pRT->waitqueueEvent);
}
...
)
{
    ...
    DDC_WAKE_UP_INTERRUPTIBLE(&pBC->sReplayBuf.waitqueueDmaCmpltEvent);
}
...
)
{
    ...
    DmaQueueEntry.BufAddr = (void *)pDeviceContext->ARINC429RxDMATarget;             /* DMA buffer */
    DmaQueueEntry.Direction = DESCRIPTOR_POINTER__DIR_OF_XFER_FROM_DEVICE;                      /* from device */
    DmaQueueEntry.SgListNumEntries = 1;                                              /* Num SGList entries */

    DmaQueueEntry.u32DMACHBaseAddr = (*(pDeviceContext->pRxChnl429[u8Ch]->pu32MemBA) << 2);     /* byte address */
    DmaQueueEntry.u32DMACHMaxAddr = ((*(pDeviceContext->pRxChnl429[u8Ch]->pu32MemBA) +
            *(pDeviceContext->pRxChnl429[u8Ch]->pu32MemSize)) << 2);                            /* byte address */
    DmaQueueEntry.u8DMAType = DMA_429_RX_FIFO;

    /* alignment check */
    DmaQueueEntry.copyOffset = 0;
    DmaQueueEntry.u32OddAlignment = FALSE;

    if ((DmaQueueEntry.ByteCount % DMA_ALIGNMENT_BYTES) != 0)
    {
        ...
        DmaQueueEntry.ByteCount -= sizeof(U32BIT);
        DmaQueueEntry.RequestedByteCount -= sizeof(U32BIT);
        DmaQueueEntry.u32OddAlignment = TRUE;
    }
...
)
{
    ...
    if (pDmaEntry->u32OddAlignment)
    {
        ...
        DDC_MEM_READ(pDeviceContext, pDmaEntry->StartAddr + u32Tail, &u32Data, ACEX_32_BIT_ACCESS);
    }
...
#if DDC_PPC
    {
        ...
        {
            pData[byteSwapIndex] = BYTE_ORDER_L(pData[byteSwapIndex]);
        }
    }
#endif /* DDC_PPC */
    ...
    if (pDmaEntry->u32OddAlignment)
    {
        ...
        pDmaEntry->userBuf[u32DestIndex] = u32Data;
    }
    ...
    DDC_WAKE_UP_INTERRUPTIBLE(&pDeviceContext->pRxChnl429[u8Ch]->waitqueueARINC429RxFifo);
}
...
)
{
    ...
    DDC_WAKE_UP_INTERRUPTIBLE(&pDeviceContext->pTxChnl429[u8Ch]->waitqueueARINC429SetTxFrameControl);
}
...
)
{
...
#if DDC_PPC
    {
        ...
        for (byteSwapIndex = 0; byteSwapIndex < pDmaEntry->RequestedByteCount / sizeof(U32BIT); byteSwapIndex++)
        {
            ...
            pData[byteSwapIndex] = WORD_ORDER_L(pData[byteSwapIndex]);
        }
    }
#endif /* DDC_PPC */
    ...
    DDC_WAKE_UP_INTERRUPTIBLE(&pDeviceContext->waitqueueArinc429VoltageMonitoring);
}
...
)
{
    ...
    DDC_WAKE_UP_INTERRUPTIBLE(&pDeviceContext->waitqueueARINC717RxFrame[u8Ch]);
}
...
)
{
    ...
    DDC_WAKE_UP_INTERRUPTIBLE(&pDeviceContext->waitqueueARINC717TxFrame[u8Ch]);
}
...
)
{
    ...
    DDC_WAKE_UP_INTERRUPTIBLE(&pDeviceContext->sDioTt.dmaCmpltWaitqueueEvent);
}
...
)
{
    ...
    u16DmaChannel = pDeviceContext->sDMA.queueEntryCurrentTransaction.Channel;
...
)
{
    ...
    ptr = (DESCRIPTOR_POINTER_ADDR(DDC_VIRT_TO_PHYS(pDeviceContext->sDMA.pu32Descriptor)) << 4) | PLX_DMA_DESC_PTR_PCI_ADDR;

    DDC_DBG_PRINT(DDC_DBG_MODULE_DMA, DDC_DBG_DMA_START, "write REG_QPRM_DMA_DSC_PTR ADDR = 0x%x\n",
        DESCRIPTOR_POINTER_ADDR(DDC_VIRT_TO_PHYS(pDeviceContext->sDMA.pu32Descriptor) << 4));
...
)
{
    ...
    ptr = (U32BIT)DDC_VIRT_TO_PHYS(pDeviceContext->sDMA.pu32Descriptor);
    ...
    if (pDeviceContext->sDMA.u32Direction == DESCRIPTOR_POINTER__DIR_OF_XFER_TO_DEVICE)
    {
        ...
        DDC_DBG_PRINT(DDC_DBG_MODULE_DMA, DDC_DBG_DMA_START, "DMA to device: write Descriptor Pointer ADDR = 0x%x\n",
            DESCRIPTOR_POINTER_PCIE_ADDR(DDC_VIRT_TO_PHYS(pDeviceContext->sDMA.pu32Descriptor)));
...
)
{
    ...
    dtePhysicalAddress = (U32BIT)(DDC_VIRT_TO_PHYS(dteVirtualAddress));
    dtePhysicalAddress += (U32BIT)(sizeof(DMA_TRANSFER_ELEMENT_PCIE));
...
#if DDC_PPC
{
    ...
    dtePhysicalAddressTemp = (U32BIT)DDC_VIRT_TO_PHYS(dteVirtualAddress) + sizeof(DMA_TRANSFER_ELEMENT_PCIE);
    ...
    for (i = 0; i < pDMATransaction->SgListNumEntries; i++)
    {
        ...
        /* dteVirtualAddressTemp->PciAddressHigh is always 0, so there is no need to swap */
        dteVirtualAddressTemp->TransferSize = BYTE_ORDER_L(dteVirtualAddressTemp->TransferSize);
        dteVirtualAddressTemp->DmaNextDescriptorPointerLSB = BYTE_ORDER_L(dteVirtualAddressTemp->DmaNextDescriptorPointerLSB);
        dteVirtualAddressTemp->DmaNextDescriptorPointerMSB = BYTE_ORDER_L(dteVirtualAddressTemp->DmaNextDescriptorPointerMSB);

        /*Adjust the next DMA_TRANSFER_ELEMEMT */
        dteVirtualAddressTemp++;
        dtePhysicalAddressTemp += sizeof(DMA_TRANSFER_ELEMENT_PCIE);
    }
}
#endif /* DDC_PPC */
...
#if DDC_PPC
    /* unswap value */
    dteVirtualAddress->PciAddressLow = BYTE_ORDER_L(dteVirtualAddress->PciAddressLow);
#endif /* DDC_PPC */
