...
if (len(sys.argv) != 3):
    print 'usage == python 1)Command Name 2)Interval time '
    exit(0)
    ...
    for i in range(len(input_file_list)):
        ...
        for y in range(line_count):
            ...
            if bytes:
                ...
                c1  = data[0]
                if c1-c2 > float(sys.argv[2])/1000:
                        print 'line number', y
                c2 = c1
                ...
                try:
                    for k in range(len(index_list[index[i]])):
                        ...
                        if (scale_list[index[i]][k] == -1):
                            ...
                            val = float(int(data[index_list[index[i]][k]]) << 16 | int(
                                data[index_list[index[i]][k + 1]])) * 0.0025
        ...
        print "End"
        fig = figure(1)
        fig.canvas.set_window_title('plot at ')
